...
int init_mc_ml(pg_flow_ml *gf, char *ifile) {
    ...
    init_hb_multilevel(pg_var_ml.ml_levels, pg_var_ml.beta, pg_var_ml.nhb, pg_var_ml.nor, pg_var_ml.ml_niteration,
                       pg_var_ml.ml_nskip, pg_var_ml.nblkstart, pg_var_ml.nblkend, pg_var_ml.APEsmear, &(pg_var_ml.corrs));
...
int init_mc_ml_measure(pg_flow_ml_measure *gf, char *ifile) {
    ...
    error(pg_var_ml.tune_lev < -1 || pg_var_ml.tune_lev >= pg_var_ml.ml_levels, 1, "init_mc_ml_measure",
          "Op tune level must be 0 <= tune_lev < ml_levels\n");

    pg_var_ml.ml_niteration = malloc(sizeof(int) * pg_var_ml.ml_levels);
    pg_var_ml.ml_nskip = malloc(sizeof(int) * pg_var_ml.ml_levels);

    parse_ml_corellator_def();
    if (pg_var_ml.tune_lev != -1) { lprintf("INIT ML", 0, "ML tuning level=%d\n", pg_var_ml.tune_lev); }
