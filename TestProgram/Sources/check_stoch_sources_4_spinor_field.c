...
static int test_passed(double mean, double stdev)
{
  ...
  if (fabs(mean) < 1e-10 && fabs(stdev) < 1e-10)
    return 0;
  if (mean < 0. && mean + stdev > 0.)
    return 0;
  if (mean >= 0. && mean - stdev < 0.)
    return 0;
...
int main(int argc, char *argv[])
{
  ...
  for (i = 0; i < mes_ip.nhits; i++)
  {
    ...
    for (j = 0; j < 4; ++j)
    {
      ...
      if (fabs(norm / ((double)NF * GLB_X * GLB_Y * GLB_Z) - 1.) > 1e-14)
        return_value += 1;
    } // loop on j
  }
  ...
  for (col = 0; col < NF; ++col)
  {
    for (i = 0; i < mes_ip.nhits; i++)
    {
      ...
      for (j = 0; j < 4; ++j)
      {
        ...
        if (fabs(norm / ((double)GLB_X * GLB_Y * GLB_Z) - 1.) > 1e-14)
          return_value += 1;
      }
    }
  }
  ...
  for (col = 0; col < NF; ++col)
  {
    for (i = 0; i < mes_ip.nhits; i++)
    {
      ...
      for (eo = 0; eo < 2; ++eo)
      {
        ...
        {
          norm = spinor_field_sqnorm_f(&source[j]);
          if (fabs(norm / (0.5 * (double)GLB_X * GLB_Y * GLB_Z) - 1.) > 1e-14)
            return_value += 1;
        } // loop on j
      }   // loop on eo.
    }
