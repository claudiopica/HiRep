...
int main(int argc,char *argv[])
{
  ...
  for (int i=0;i<mes_ip.nhits;i++){
    ...
    {
      printf("%e %e %e\n",norm,(double) 4*NF*GLB_X*GLB_Y*GLB_Z*GLB_T,norm-(double) 4*NF*GLB_X*GLB_Y*GLB_Z*GLB_T);
      return_value+=1;
    }
  }
    ...
    for (int i=0;i<mes_ip.nhits;i++) {
      ...
      for(x =  0; x < X; x += 1)   for(y = 0; y < Y; y += 1)  for(z = 0; z < Z; z += 1)    for(t = 0; t < T; t += 1) {
        ...
        for (b1=0;b1<4;++b1) for (c1=0; c1<NF; ++c1) for (b2=0;b2<4;++b2) for (c2=0; c2<NF; ++c2){
          tmp_vec[counter] += conj(_FIELD_AT(source,ix)->c[b1].c[c1]) * _FIELD_AT(source_shifted,ix)->c[b2].c[c2] - DeltaKronecker(tmp_index,0)*DeltaKronecker(b1,b2)*DeltaKronecker(c1,c2)  ; // that quantity should be 0 in average over the noise.
        } //loop local volume
      }
      counter+=1;
    }//loop nhits
