...
bool test_hermiticity(spinor_operator S, spinor_operator S_cpu, char *name)
{
    ...
    bool pass_cpu = is_hermitian_on_CPU(s1, s2, S_s1, S_s2, S_cpu);

    bool pass_sanity_check = result_spinor_fields_not_identically_zero(S_s1, S_s2);
    bool are_copies_identical = gpu_and_cpu_copies_identical(S_s1, S_s2);

    free_spinors(&s1, &s2, &S_s1, &S_s2);
    return pass_gpu && pass_cpu && pass_sanity_check && are_copies_identical;
}
...
bool result_spinor_fields_not_identically_zero(spinor_field *S_s1, spinor_field *S_s2) 
{
    bool pass_gpu = spinor_field_sqnorm_f(S_s1)!=0 && spinor_field_sqnorm_f(S_s2)!=0;
    bool pass_cpu = spinor_field_sqnorm_f_cpu(S_s1)!=0 && spinor_field_sqnorm_f_cpu(S_s2)!=0;
    if (!pass_gpu) 
    {
