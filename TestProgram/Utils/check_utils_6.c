...
int main(int argc, char *argv[])
{
    ...
    lprintf("MAIN", 0, "Checking difference between _suNf_multiply_default and _suNf_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[0]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[0]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNf_inverse_multiply_default and _suNf_inverse_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[1]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[1]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNf_double_multiply_default and _suNf_double_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[2]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[2]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNf_double_inverse_multiply_default and _suNf_double_inverse_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[3]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[3]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNg_multiply_default and _suNg_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[4]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[4]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNg_inverse_multiply_default and _suNg_inverse_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[5]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[1]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNg_double_multiply_default and _suNg_double_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[6]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[2]) > 10.e-14)
        return_value++;

    lprintf("MAIN", 0, "Checking difference between _suNg_double_inverse_multiply_default and _suNg_double_inverse_multiply_AVX2.\n ");
    lprintf("MAIN", 0, "Maximal difference = %.14e\n", fabs(err[7]));
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (fabs(err[3]) > 10.e-14)
        return_value++;
