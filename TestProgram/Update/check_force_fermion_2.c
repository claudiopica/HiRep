...
int main(int argc, char *argv[]) {
    ...
    copy_from_gpu_avfield(force_tmp);

    lprintf("SANITY", 0, "Force GPU: %0.2e\n", sqnorm_avfield_cpu(force_tmp));
    lprintf("SANITY", 0, "Force CPU: %0.2e\n", sqnorm_avfield_cpu(force));
    ...
    sub_assign_avfield_cpu(force, force_tmp);
    sqnorm = sqrt(sqnorm_avfield_cpu(force));
    return_val += check_diff_norm(sqnorm, 1e-12);
