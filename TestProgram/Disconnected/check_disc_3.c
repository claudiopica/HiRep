...
static double complex * read_and_average_output_disc(int ixG,int nhits){
  ...
  strcat(command_re, " out_0 | awk  '{sum+=$5;sum2+=$5*$5}END{print sum, sum2;}' ") ;
  strcat(command_im, " out_0 | awk  '{sum+=$6;sum2+=$6*$6}END{print sum, sum2;}' ") ;
  ...
  out[0]= (tmp_re + I*tmp_im)/(NF*GLB_T*nhits) ; // mean
  out[1] = sqrt(tmp2_re/(NF*GLB_T*nhits) - creal(out[0])*creal(out[0]))  + I*sqrt(tmp2_im/(NF*GLB_T*nhits) - cimag(out[0])*cimag(out[0])) ; // standard deviation

  // This is to return the average and standard deviation of the sum over color.
  out[0] *= (double) NF;
  out[1] *= (double) NF;
  return out ;
}
...
int main(int argc,char *argv[])
{
  ...
  {
    lprintf("MAIN",0,"Percentage of error for the scalar loop at zero momentum: %e\n",  fabs(creal(ex_loops[8] - loops[8][0])/creal(ex_loops[8])));
    return_value +=1;
  }
