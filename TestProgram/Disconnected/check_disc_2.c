...
* File check_disc_2.c
*
* Check of the  disc loops (free case): discon volume source (type = 2)
*
...
static double complex * read_and_average_output_disc(int ixG,int nhits){
  ...
  strcat(command_re, " out_0 | awk  '{sum+=$4;sum2+=$4*$4}END{print sum, sum2;}' ") ;
  strcat(command_im, " out_0 | awk  '{sum+=$5;sum2+=$5*$5}END{print sum, sum2;}' ") ;
