...
#define init_input_mesons(varname)                                                            \
    {                                                                                         \
        .read = {                                                                             \
            ...
            { "number of inversions per cnfg", "disc:nhits2 = %d", INT_T, &(varname).nhits }, \
    ...
    for (int k = 0; k < mes_ip.nhits; k++) {
        for (int j = 0; j < n_Gamma; j++) {
            for (int t = 0; t < GLB_T; t++) {
                ...
                mean_loops[j] += (*data_storage_element(out_corr, 0, idx_re) + I * *data_storage_element(out_corr, 0, idx_im)) /
                                 (mes_ip.nhits * GLB_T);
            }
        }
    }

    ex_loops = (hr_complex *)malloc(16 * sizeof(hr_complex));
