    ...
    for (int n = 0; n < nGamma; n++) {
        if (cabs(corr_ex[n] - corr_num[n]) > tol) {
            lprintf("TEST", 0, "Mismatch %s, ip=%d, absolute diff: %e, numeric = %e + I*(%e), analytic = %e + I*(%e) \n",
                    name[n], ip, cabs(corr_ex[n] - corr_num[n]), creal(corr_num[n]), cimag(corr_num[n]), creal(corr_ex[n]),
                    cimag(corr_ex[n]));
            retval += 1;
        } else {
            lprintf("TEST", 0, "Match %s, ip=%d, numeric = %e + I*(%e), analytic = %e + I*(%e) \n", name[n], ip,
                    creal(corr_num[n]), cimag(corr_num[n]), creal(corr_ex[n]), cimag(corr_ex[n]));
        }
    }
    return retval;
}
    ...
    for (px = 0; px < n_mom; ++px) {
        for (py = 0; py < n_mom; ++py) {
            for (pz = 0; pz < n_mom; ++pz) {
                ...
                for (i = 0; i < 16; i++) {
                    loops[ll][i] = phase * (gid[i] * A - I * (g0[i] * B[0] + g1[i] * B[1] + g2[i] * B[2] + g3[i] * B[3])) *
