...
int main(int argc, char *argv[]) {
    ...
    for (n = 0; n < T * total_n_tor_op; n++) {
        if (fabs(creal(dop1[n])) > max_diff[0]) { max_diff[0] = fabs(creal(dop1[n])); }

        if (fabs(cimag(dop1[n])) > max_diff[1]) { max_diff[1] = fabs(cimag(dop1[n])); }

        if (fabs(creal(dop1[n])) > 1.e-12) { return_value++; }
        if (fabs(cimag(dop1[n])) > 1.e-12) { return_value++; }
    ...
    lprintf("MAIN", 0, "(should be around 1*10^(-15) or so)\n\n");
    if (sqrt(creal(ca) * creal(ca) + cimag(ca) * cimag(ca)) > 1.e-12) { return_value++; }
