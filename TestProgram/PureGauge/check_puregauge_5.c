...
int main(int argc, char *argv[]) {
    ...
    for (n = 0; n < T * total_n_glue_op; n++) {
        if (fabs(creal(dop[n])) > max_diff[0]) { max_diff[0] = fabs(creal(dop[n])); }

        if (fabs(cimag(dop[n])) > max_diff[1]) { max_diff[1] = fabs(cimag(dop[n])); }
        ...
        if (fabs(creal(dop[n])) > 10.e-14) { return_value++; }
        if (fabs(cimag(dop[n])) > 10.e-14) { return_value++; }
