...
int main(int argc,char *argv[])
{
  ...
  int numsources = mes_var.nhits;
        ...
        lprintf("MAIN",0,"csw is now set to %e: ", mes_var.csw);
  	#endif
   ...
   for (int n=0;n<numsources;n++)
    {   
        ...
        measure_pion_scattering_I0( m,1 ,mes_var.precision,NULL,"test",1);

        D=get_meson_observables_D();
        C=get_meson_observables_C();
        R=get_meson_observables_R();
        V=get_meson_observables_V();
        
        // now copy the content to a placeholder.
        for (int t=0;t < GLB_T ; t++) 
            {
                Dstoch[t] += 2*(D->corr_re[corr_ind(0,0,0,0,t,1,0)]+I*D->corr_im[corr_ind(0,0,0,0,t,1,0)])/(numsources*GLB_VOLUME); // the factor 2 is usually in io2pt
                Cstoch[t] += 2*(C->corr_re[corr_ind(0,0,0,0,t,1,0)]+I*C->corr_im[corr_ind(0,0,0,0,t,1,0)])/(numsources*GLB_VOLUME); // the factor 2 is usually in io2pt
                Vstoch[t] += 4*(V->corr_re[corr_ind(0,0,0,0,t,1,0)]+I*V->corr_im[corr_ind(0,0,0,0,t,1,0)])/(numsources*GLB_X*GLB_Y*GLB_Z); // the factor 2 is usually in io2pt
                Rstoch[t] += 4*(R->corr_re[corr_ind(0,0,0,0,t,1,0)]+I*R->corr_im[corr_ind(0,0,0,0,t,1,0)])/(numsources*GLB_VOLUME); // the factor 4 is usually in io4pt
           }
        for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"tmp C numerical %e + 1I %e\n" ,creal(Cstoch[i]), cimag(Cstoch[i]));
    ...
    for (int t=0;t < GLB_T ; t++)  
    {
        Dtheo[t] = cpow(twopoint(zero_p, m[0],GLB_X, GLB_T, t),2);
        Ctheo[t] = cpow(twopoint(zero_p, m[0],GLB_X, GLB_T, t),2);
        Vtheo[t] = cpow(twopoint(zero_p, m[0],GLB_X, GLB_T, 0),2);
        Rtheo[t] = R_(zero_p,zero_p,zero_p,m[0],GLB_X,GLB_T,t);
    }
  
  
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"V analytical V %e + 1I %e  numerical %e + 1I %e\n" ,creal(Vtheo[i]),cimag(Vtheo[i]),creal(Vstoch[i]), cimag(Vstoch[i]));
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"C analytical C %e + 1I %e  numerical %e + 1I %e\n" ,creal(Ctheo[i]),cimag(Ctheo[i]),creal(Cstoch[i]), cimag(Cstoch[i]));
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"D analytical D %e + 1I %e  numerical %e + 1I %e\n" ,creal(Dtheo[i]),cimag(Dtheo[i]),creal(Dstoch[i]), cimag(Dstoch[i]));
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"R analytical R %e + 1I %e  numerical %e + 1I %e\n" ,creal(Rtheo[i]),cimag(Rtheo[i]),creal(Rstoch[i]), cimag(Rstoch[i]));
   

    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"diff analytical R %e + 1I %e \n" ,creal(Rtheo[i]) - creal(Rstoch[i]),cimag(Rtheo[i])- cimag(Rstoch[i]));
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"diff analytical V %e + 1I %e \n" ,creal(Vtheo[i]) - creal(Vstoch[i]),cimag(Vtheo[i])- cimag(Vstoch[i]));
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"diff analytical C %e + 1I %e \n" ,creal(Ctheo[i]) - creal(Cstoch[i]),cimag(Ctheo[i])- cimag(Cstoch[i]));
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"diff analytical D %e + 1I %e \n" ,creal(Dtheo[i]) - creal(Dstoch[i]),cimag(Dtheo[i])- cimag(Dstoch[i]));
