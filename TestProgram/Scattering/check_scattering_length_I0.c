...
int main(int argc,char *argv[])
{
 ...
 // Now compare R and Ralt
    ...
    for (int t=0;t < GLB_T ; t++)   Raltstoch[t] = (mo_arr[14]->corr_re[corr_ind(0,0,0,0,t,1,0)]+I*mo_arr[14]->corr_im[corr_ind(0,0,0,0,t,1,0)]); 
    for (int i=0;i<GLB_T;++i)    lprintf("MAIN",0,"R analytical Ralt %e + 1I %e  numerical %e + 1I %e  ratio (num/theo) %e + 1I %e\n" ,creal(Rtheo[i]),cimag(Rtheo[i]),creal(Raltstoch[i]), cimag(Raltstoch[i]),creal(Raltstoch[i]/Rtheo[i]),cimag(Raltstoch[i]/Rtheo[i]));
