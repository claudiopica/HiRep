...
}; /* g0 = tr gamma_0 Gamma gamma_0 \bar{Gamma} / 4  Because our matrices Gamma are not necessary hermitiean !*/
...
char *mes_channel_names[16] = { "g5",   "id",   "g0",   "g1",   "g2",   "g3",     "g0g1",   "g0g2",
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < 16; i++) {
        ...
        for (t = 0; t < GLB_T; t++) {
            lprintf("TEST", 0, "%e\t%e\t%e\n", ex_triplets[i][t], creal(corr_triplets[i][t]),
                    fabs(ex_triplets[i][t] - creal(corr_triplets[i][t])));
            if (fabs(ex_triplets[i][t] - creal(corr_triplets[i][t])) > tol) { return_value += 1; }
            if (fabs(cimag(corr_triplets[i][t])) > tol) { return_value += 1; }
        }
    }
