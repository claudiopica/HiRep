...
int main(int argc,char *argv[])
{
  ...
  measure_spectrum_semwall(1,&mass,mes_ip.nhits_2pt,0,1e-14);
...
double complex z;
  z =  mass+4.0+cos((2.0*M_PI*k[0])/GLB_T)+cos((2.0*M_PI*k[1])/GLB_X)+cos((2.0*M_PI*k[2])/GLB_Y)+cos((2.0*M_PI*k[3])/GLB_Z) +I*sqrt(
  ...
  for(k[1] = sigma[1]; k[1] < GLB_X+sigma[1]-.5; k[1] += 1.)
    for(k[2] = sigma[2]; k[2] < GLB_Y+sigma[2]-.5; k[2] += 1.)
      for(k[3] = sigma[3]; k[3] < GLB_Z+sigma[3]-.5; k[3] += 1.) {
	       ...
	       for(t = 0; t < GLB_T; t++) {
           A2[t] += A[t]*conj(A[t])*z;
           for (j=0;j <4; j++)  B2[j][t] += B[j][t]*conj(B[j][t])*z;
         }
