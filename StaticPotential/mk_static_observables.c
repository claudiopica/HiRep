...
int main(int argc, char *argv[]) {
...
#endif
    if (strcmp(bcpar_var.SF_bkg, "true") == 0) { BCs_pars.SF_BCs = 1; }
    ...
    if (strcmp(ospar_var.wilsonmake, "true") == 0) {
        ...
        WL_initialize();
        for (i = 0; i < 10; i++) {
            if (WL_var.c[i][0] * WL_var.c[i][0] + WL_var.c[i][1] * WL_var.c[i][1] + WL_var.c[i][2] * WL_var.c[i][2] != 0 &&
                WL_var.nsteps[i] != 0) {
    ...
    while (++i) {
        ...
        if (strcmp(ospar_var.polymake, "true") == 0) { polyakov(); }

        if (strcmp(ospar_var.wilsonmake, "true") == 0) { WL_wilsonloops(HYP_var.weight); }
