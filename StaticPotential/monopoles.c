...
char output_filename[256] = "MONOPOLES.out";
...
void read_cmdline(int argc, char* argv[]) {
  ...
  error((ac==0 && al==0) || (ac!=0 && al!=0),1,"parse_cmdline [monopoles.c]",
      "Syntax: monopole { -c <config file> | -l <list file> } [-i <input file>] [-o <output file>] [-m]");
  ...
  if(ac != 0) {
    ...
    error((list=fopen(list_filename,"r"))==NULL,1,"parse_cmdline [monopoles.c]" ,
	"Failed to open list file\n");
    error(fscanf(list,"%s",cnfg_filename)==0,1,"parse_cmdline [monopoles.c]" ,
	"Empty list file\n");
    fclose(list);
  }
...
int main(int argc,char *argv[]) {
  ...
  error(fpars.type==UNKNOWN_CNFG,1,"monopoles.c","Bad name for a configuration file");
  error(fpars.nc!=NG,1,"monopoles.c","Bad NG");
