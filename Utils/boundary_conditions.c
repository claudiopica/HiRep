...
void apply_bc_dble(){
#ifdef ANTIPERIODIC_BC_T
	{
		...
		for (ix=0;ix<L;++ix){
			for (iy=0;iy<L;++iy){
				for (iz=0;iz<L;++iz){
					suNf_dble *u=pu_gauge_f_dble(ipt[T-1][ix][iy][iz],0);
					_suNf_minus(*u,*u);
				}
			}
		}
...
#ifdef ANTIPERIODIC_BC_X
	{
		...
		for (it=0;it<T;++it){
			for (iy=0;iy<L;++iy){
				for (iz=0;iz<L;++iz){
					suNf_dble *u=pu_gauge_f_dble(ipt[it][L-1][iy][iz],1);
					_suNf_minus(*u,*u);
				}
			}
		}
...
#ifdef ANTIPERIODIC_BC_Y
	{
		...
		for (it=0;it<T;++it){
			for (ix=0;ix<L;++ix){
				for (iz=0;iz<L;++iz){
					suNf_dble *u=pu_gauge_f_dble(ipt[it][ix][L-1][iz],2);
					_suNf_minus(*u,*u);
				}
			}
		}
...
#ifdef ANTIPERIODIC_BC_Z
	{
		...
		for (it=0;it<T;++it){
			for (ix=0;ix<L;++ix){
				for (iy=0;iy<L;++iy){
					suNf_dble *u=pu_gauge_f_dble(ipt[it][ix][iy][L-1],3);
					_suNf_minus(*u,*u);
				}
			}
		}
