...
void check_list_dir_and_data_file(par *apar) {
    ...
    if (apar->Dflag == 1) {
        ...
        if (file_with_dirs.good()) {
            ...
            std::cerr << "[ERROR][CHECK_LIST_DIR_AND_DATA_FILE] The file \"" << apar->list_dir << "\" does not exist!"
                      << std::endl;
            exit(1);
        }
    ...
    for (std::vector<datafile>::iterator it_file = apar->vevfiles.begin(); it_file != apar->vevfiles.end(); ++it_file) {
        ...
        if (!data_file.good()) {
            std::cerr << "[ERROR][CHECK_LIST_DIR_AND_DATA_FILE_AND_DATA_FILE] The file \"" << it_file->filename
...
void print_parameters(const par *apar) {
    ...
    std::cout << "      Largest allocation size in Gb ->  "
              << double(apar->numop * apar->numop * apar->ndt * apar->numbinjk * sizeof(double)) / 1073741824 << std::endl;
}
...
static void check_file_data(std::ifstream &stream_file, int *opcount, int *nidx2, int *nidx1) {
    ...
    if (*opcount == 0) { *opcount = op_count; }

    if (*opcount != op_count) {
        std::cerr << "[ERROR][check_file_data] Different number of operators " << *opcount << " " << op_count
                  << " between files!" << std::endl;
        exit(1);
    }
    ...
    do {
        ...
        if (stream_file.eof()) { break; }
    } while (true);

    if (*nidx2 == 0) { *nidx2 = nrep; }

    if (*nidx2 != nrep) {
        std::cerr << "[ERROR][check_file_data] Different number nrep " << *nidx2 << " " << nrep
                  << "(second index) between files!" << std::endl;
        exit(1);
    }
...
void read_2pt_def(par *apar) {
    ...
    if (file_with_defs.good()) {
                ...
                while (true) {
                    ...
                    while (true) {
                        ...
                        if (sscanf(subsubline.c_str(), "%d-%d", &p1, &p2) == 2) {
                            if (p2 <= p1) {
                                std::cerr
                                    << "[ERROR][read_2pt_def] Wrong format in the corellator definition string. \n The correlators are inputed according to the structure x1-y1|x2-y2,x3-y3 with yi>xi and pair separated by the | must have the same distance"
                                    << std::endl;
                                exit(1);
                            }
                            if (dt == 0 || dt == p2 - p1) {
        ...
        for (std::map<int, dtcor>::iterator it_dt = apar->corrdef.begin(); it_dt != apar->corrdef.end(); ++it_dt) {
            if (it_dt->first < mindt) { mindt = it_dt->first; }
            it_dt->second.index = idt;
            idt++;
        }
        if (apar->xflag == 1) {
            if (apar->corrdef.find(apar->p_inv) == apar->corrdef.end()) {
                std::cerr << "[ERROR][READ_2PT_DEF] The inversion distance is not measured in  " << apar->cor_def_filename
                          << std::endl;
                exit(1);
            }
        } else {
        ...
        if (apar->dflag == 1) {
            if (apar->corrdef.find(apar->p_diag) == apar->corrdef.end()) {
                std::cerr << "[ERROR][READ_2PT_DEF] The Diagionalization distance is not measured in  "
                          << apar->cor_def_filename << std::endl;
                exit(1);
            }
        } else {
            ...
            for (int l = 0; l < apar->binwidth; l++) {
                for (int j = 0; j < apar->nt; j++) {
                    ...
                    for (int op1 = 0; op1 < apar->numop; op1++) {
                        ...
                        vev_dataout[op1 + apar->numop * (j + apar->nt * measurecount)] +=
                            localdata[op1 + apar->numop * j] * norm_vev;
                    }
                }

                countdt = 0;
                for (std::map<int, dtcor>::iterator if_dist = apar->corrdef.begin(); if_dist != apar->corrdef.end();
                     ++if_dist) {
                    norm_cor = 1.0 / (apar->binwidth * if_dist->second.points.size());
                    for (std::vector<cpoints>::iterator pair = if_dist->second.points.begin();
                         pair != if_dist->second.points.end(); ++pair) {
                        ...
                        for (int op1 = 0; op1 < apar->numop; op1++) {
                            for (int op2 = 0; op2 < apar->numop; op2++) {
                                cor_dataout[op2 + apar->numop * ((op1) + apar->numop * (countdt + measurecount * apar->ndt))] +=
                                    norm_cor *
                                    real(conj(localdata[op1 + apar->numop * it1]) * localdata[op2 + apar->numop * it2]);
                            }
                        }
                    }
                    countdt++;
                ...
                for (int bin = 0; bin < apar->numbinjk; bin++) {
                    ...
                    if (apar->vflag == 1) {
                        for (std::vector<cpoints>::iterator pair = if_dist->second.points.begin();
                            ...
                            it2 = apar->t2idx[pair->p2];
                            cor_jk[op2 + apar->numop * ((op1) + apar->numop * (dtcounter + bin * apar->ndt))] -=
                                vev_jk[op2 + apar->numop * (it2 + apar->nt * bin)] *
                                vev_jk[op1 + apar->numop * (it1 + apar->nt * bin)] / if_dist->second.points.size();
                        }
                    }
                }
            ...
            for (int t = 0; t < apar->ndt; t++) {
                for (int bin = 0; bin < apar->numbinjk; bin++) {
                    cor_jk[op2 + apar->numop * ((op1) + apar->numop * (t + bin * apar->ndt))] =
                        norm * (cor_jk[op2 + apar->numop * ((op1) + apar->numop * (t + bin * apar->ndt))] +
...
void deactivate_pull_corr_jk(par *apar, double *cor_jk, double pull, int dt) {
    ...
    for (int op1 = 0; op1 < apar->numop; op1++) {
        ...
        if (corjk_avg / corjk_var < pull) { deactivate_op(apar, op1); }
    }
}
