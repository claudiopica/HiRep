...
void fitexp(par *apar, double *diagzed_corr) {
    ...
    double anorm = 0.0, snorm = 0.0, amm, amu, aml, avn, avd, ff, etry, el, eu, abest, chis, chi, ebest, cof, sen, aen, scoef,
           acoef, schisq, achisq;
    int numdel;
    ...
    for (std::map<int, dtcor>::iterator it_dt = apar->corrdef.begin(); it_dt != apar->corrdef.end(); ++it_dt) {
        ...
        std::cout << "\t cor[ " << it_dt->first << " ]= " << acor[nt] * anorm << " +/- "
                  << (apar->numbinjk - 1) / (apar->numbinjk * sqrt(apar->numbinjk)) * scor[nt] << std::endl;
    }

    if (scor[0] < small) { scor[0] = scor[1]; }
    for (int nt = 0; nt < apar->ndt; nt++) {
        if (scor[nt] < small) { scor[nt] = 1.0; }
    }

    std::cout << std::endl << "[INFO][fitexp] Mass fit " << std::endl;

    int iOBC = 0;
    if (apar->Tflag == 1) { iOBC = 1; }
        ...
        for (int itu = itl + 1; itu < apar->ndt; itu++) {
            for (int ib = 0; ib < apar->numbinjk; ib++) {
                for (int idel = 0; idel < numdel; idel++) {
                    ...
                    if (avd < small) { avd = small; }
                    abest = avn / avd;
                    chis = 0.0;
                    for (int idt = itl; idt < itu + 1; idt++) {
                        ff = abest * (exp(-etry * idt) + iOBC * exp(-etry * (apar->lt - idt)));
                        chis +=
                            ((acorn[idt + apar->ndt * ib] - ff) / scor[idt]) * ((acorn[idt + apar->ndt * ib] - ff) / scor[idt]);
                    }
                    if (idel == 0) {
                        chi = chis;
                        ebest = etry;
            ...
            if (sen > 0) { sen = sqrt(sen); }
            if (scoef > 0) { scoef = sqrt(scoef); }
            if (schisq > 0) { schisq = sqrt(schisq); }

            std::cout << std::fixed << std::setprecision(6);
            std::cout << "dt range " << itl << "-" << itu << "\tmass= " << aen << " " << sen << " \tamplitude= " << acoef << " "
                      << scoef << "\t\tchisq= " << achisq << " " << schisq << std::endl;
            if (achisq > 20) { itu = apar->ndt; }
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
