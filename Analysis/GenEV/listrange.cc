...
bool oplist::add_op() {
    ...
    if (this->back().is_set) {
        ...
        if (this->back().good_bl.empty()) { this->back().is_set = true; }
    }
...
void oplist::report() {
    ...
    int blrange[2] = { 0, 0 };
    for (it1 = this->begin(); it1 != this->end(); it1++) {
        if (it1->good_bl.empty()) { continue; }
        int op = it1->op_index;
        if (oprange[0] == -1) { oprange[0] = oprange[1] = op; }
        bool fullop = true;
        std::sort(it1->good_bl.begin(), it1->good_bl.end());

        if ((int)(it1->good_bl.size()) != Bl_max - Bl_min + 1) { fullop = false; }
    ...
    if (!myrep.empty()) { myrep.erase(myrep.end() - 1); }
            ...
            for (it2 = it1->good_bl.begin(); it2 != it1->good_bl.end(); it2++) {
                for (it3 = this->begin(); it3 != this->end(); it3++) {
                    for (it4 = it3->good_bl.begin(); it4 != it3->good_bl.end(); it4++) {
                        ret[id] = mat[(it1->op_index - 1) + (*it2 - 1) * numop + ((it3->op_index) - 1) * numop * nblock +
                                      (*it4 - 1) * numop * nblock * numop + iout * nblock * numop * nblock * numop];
                        id++;
                    }
                }
            }
