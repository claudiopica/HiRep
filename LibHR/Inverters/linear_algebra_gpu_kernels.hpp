...
/* Re <s1,s1> */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_sqnorm_gpu(SITE_TYPE *s1, double *resField, int N) {
    SITE_TYPE site1;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site1, s1, ix, 0, 1);
...
/* s1=r*s2 r real */
template <typename SITE_TYPE, typename REAL> __global__ void spinor_field_mul_gpu(SITE_TYPE *s1, REAL r, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
/* s1+=s2 */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_add_assign_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site1, s1, ix, 0, 1);
...
/* s1-=s2 */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_sub_assign_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site1, s1, ix, 0, 1);
...
/* s1=0 */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_zero_gpu(SITE_TYPE *s1, int N) {
    SITE_TYPE site;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        _spinor_zero_f(site);
...
/* s1=-s2 */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_minus_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
/* s1=g5*s2  */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_g5_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
/* s1=g5*s1 */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_g5_assign_gpu(SITE_TYPE *s1, int N) {
    SITE_TYPE site1;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site1, s1, ix, 0, 1);
...
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_g0_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_g1_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_g2_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_g3_gpu(SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site2, s2, ix, 0, 1);
...
/* tools per eva.c  */
template <typename SITE_TYPE, typename REAL> __global__ void spinor_field_lc1_gpu(REAL r, SITE_TYPE *s1, SITE_TYPE *s2, int N) {
    SITE_TYPE site1;
    SITE_TYPE site2;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site1, s1, ix, 0, 1);
...
/* s1=-s1 */
template <typename REAL, typename SITE_TYPE> __global__ void spinor_field_minus_assign_gpu(SITE_TYPE *s1, int N) {
    SITE_TYPE site1;
    for (int ix = blockIdx.x * blockDim.x + threadIdx.x; ix < N; ix += blockDim.x * gridDim.x) {
        read_gpu<REAL>(N, &site1, s1, ix, 0, 1);
