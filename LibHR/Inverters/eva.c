...
static void init_subsp(int nev, int nevt, int init, spinor_field *ev) {
    ...
    for (n = 0; n < nevt; n++) {
        if ((init == 0) || ((init == 1) && (n >= nev))) { gaussian_spinor_field(&ev[n]); }

        mgm_subsp(n, ev);
    }
}

static void ritz_subsp(int nlock, int nevt, spinor_operator Op, spinor_field *ws, spinor_field *ev, double d[]) {
    int neff, i, j;
    hr_complex z;
...
static int res_subsp(int nlock, int nev, double omega1, double omega2, spinor_operator Op, spinor_field *ws, spinor_field *ev,
                     double d[]) {
    int i, ia, ib;
    ...
    for (ib = nlock; ib < nev; ib++) {
        if (ib > ia) {
            ...
            if ((eps1 > omega1) && (eps1 > (omega2 * absd1))) { return ia; }
        }
        ...
        if (ib > ia) {
            if ((d[ib] - d[ib - 1]) > (eps1 + eps2)) { ia = ib; }
        }

        if ((eps2 > omega1) && (eps2 > (omega2 * absd2))) { return ia; }
...
static void apply_cheby(int k, double lbnd, double ubnd, spinor_operator Op, spinor_field *ws, spinor_field *ev) {
    int j;
    double c1, c2;
...
int eva(int nev, int nevt, int init, int kmax, int imax, double ubnd, double omega1, double omega2, spinor_operator Op,
    ...
    *status = 0;
    ...
    error((nev <= 0) || (nevt < nev) || (kmax < 2) || (imax < 1), 1, "eva [eva.c]",
          "Improper parameters nev,nevt,kmax or imax");
    error((omega1 < (ubnd * EPSILON)) && (omega2 < EPSILON), 1, "eva [eva.c]", "Improper parameters omega1 or omega2");
    ...
    for (i = 0; i < imax; i++) {
        ...
        *status = nop;
        ...
        lprintf("EVA", 10, "i=%3d, k=%3d, d[%2d]=% .6e, d[%2d]=% .6e, lbnd=% .6e, eps[%2d]=% .1e\n", i, k, n, d[n], nlst - 1,
                d[nlst - 1], lbnd, n, ee[n]);

        error(d[nlst - 1] > ubnd, 1, "eva [eva.c]", "Parameter ubnd is too low");
