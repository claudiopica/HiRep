...
static void print_global_coordinates(site_info *s) {
    ...
    lprintf("TEST_GEOMETRY", loglevel, "( %d , %d , %d , %d )\n", s->glb_coord[0], s->glb_coord[1], s->glb_coord[2],
            s->glb_coord[3]);
}
...
static void print_geometric_info(site_info *s) {
    ...
    lprintf("TEST_GEOMETRY", loglevel, "\n");
    if (s->local) {
        lprintf("TEST_GEOMETRY", loglevel, " [S%d] Local", s->index);
    } else {
        lprintf("TEST_GEOMETRY", loglevel, " [S%d] Nonlocal", s->index);
    }
    lprintf("TEST_GEOMETRY", loglevel, "\n");
}
...
static int in_glattice_q(int c[4]) {
    ...
    int flag = 0;
    for (i = 0; i < 4; i++) {
        if (c[i] < 0 && c[i] >= -buffer_thickness[i]) {
            howmanyoutside++;
        } else if (c[i] >= local_size[i] && c[i] < local_size[i] + buffer_thickness[i]) {
            howmanyoutside++;
...
static void set_originals_coords(site_info *s, int *test_q) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (nb >= 0 && nb < nindices) {
            ...
            lprintf("TEST_GEOMETRY", 0,
                    "Bad candidate index %d for coordinates (%d,%d,%d,%d), reached from (%d,%d,%d,%d) with iup[%d].\n", nb,
                    c[0], c[1], c[2], c[3], s->coord[0], s->coord[1], s->coord[2], s->coord[3], i);
            *test_q = false;
        }
    }
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (nb >= 0 && nb < nindices) {
            ...
            lprintf("TEST_GEOMETRY", 0,
                    "Bad candidate index %d for coordinates (%d,%d,%d,%d), reached from (%d,%d,%d,%d) with idn[%d].\n", nb,
                    c[0], c[1], c[2], c[3], s->coord[0], s->coord[1], s->coord[2], s->coord[3], i);
            *test_q = false;
        }
    }
...
static void initialize_sites() {
    ...
    local_size[2] = Y;
    local_size[3] = Z;
    buffer_thickness[0] = T_BORDER;
    buffer_thickness[1] = X_BORDER;
    buffer_thickness[2] = Y_BORDER;
    buffer_thickness[3] = Z_BORDER;
    periodic_q[0] = (NP_T == 1) ? true : false;
    periodic_q[1] = (NP_X == 1) ? true : false;
    periodic_q[2] = (NP_Y == 1) ? true : false;
    periodic_q[3] = (NP_Z == 1) ? true : false;
    ...
    lprintf("TEST_GEOMETRY", loglevel, "local_size = {%d,%d,%d,%d}\n", local_size[0], local_size[1], local_size[2],
            local_size[3]);
    lprintf("TEST_GEOMETRY", loglevel, "buffer_thickness = {%d,%d,%d,%d}\n", buffer_thickness[0], buffer_thickness[1],
            buffer_thickness[2], buffer_thickness[3]);
    lprintf("TEST_GEOMETRY", loglevel, "periodic_q = {%d,%d,%d,%d}\n", periodic_q[0], periodic_q[1], periodic_q[2],
            periodic_q[3]);
    ...
    for (s = sites; s < sites + nindices; s++) {
        ...
        for (s2 = s + 1; s2 < sites + nindices; s2++) {
            ...
            if (s->coord[0] == s2->coord[0] && s->coord[1] == s2->coord[1] && s->coord[2] == s2->coord[2] &&
                s->coord[3] == s2->coord[3]) {
    ...
    for (s = sites; s < sites + nindices; s++) {
        ...
        if (ipt(cx[0], cx[1], cx[2], cx[3]) != s->index) {
            lprintf("TEST_GEOMETRY", loglevel, "ERROR ipt(%d,%d,%d,%d)=%d ; but s->index=%d .\n", cx[0], cx[1], cx[2], cx[3],
                    ipt(cx[0], cx[1], cx[2], cx[3]), s->index);
            print_full_site_info(s);
            test_q = false;
        }
    }
    ...
    for (s = sites; s < sites + nindices; s++) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if (cy[0] != cx[0] + id[i][0] && cy[1] != cx[1] + id[i][1] && cy[2] != cx[2] + id[i][2] &&
                cy[3] != cx[3] + id[i][3]) {
                lprintf("TEST_GEOMETRY", loglevel, "ERROR iup[%d](%d,%d,%d,%d)=(%d,%d,%d,%d)\n", i, cx[0], cx[1], cx[2], cx[3],
                        cy[0], cy[1], cy[2], cy[3]);
                test_q = false;
            }
        }
...
static void search_first_block(block_info *block) {
    ...
    block->length = 0;
    for (s = sites; s < sites + nindices; s++) {
        if (compare_to_b_mask(s, mask2) && compare_to_parity(s, parity2) && s->c_type == ORIGINAL) { block->length++; }
    }
    ...
    for (s = starting_site; s < sites + nindices; s++) {
        ...
        if (!compare_to_b_mask(s, block->mask) || !compare_to_parity(s, block->parity)) {
            lprintf("TEST_GEOMETRY", loglevel, "Searching block parity=%d mask=(%d,%d,%d,%d) length=%d\n", block->parity,
                    block->mask[0], block->mask[1], block->mask[2], block->mask[3], block->length);
...
static void print_block_info(block_info *b) {
    ...
    if (b->receiver != NULL) { lprintf("TEST_GEOMETRY", loglevel, " [B%d] Receiver: %d\n", b->index, b->receiver->index); }
}
...
*/
    ...
    for (b = blocks; b < blocks + nblocks; b++) {
        ...
        for (b2 = blocks; b2 < blocks + nblocks; b2++) {
            ...
            if (b2->mask[0] != twinmask[0] || b2->mask[1] != twinmask[1] || b2->mask[2] != twinmask[2] ||
                b2->mask[3] != twinmask[3] || b2->parity != twinparity || b2->length != b->length) {
                continue;
            }
    ...
    for (k = 0; k < nindices; k++) {
        for (n = 0; n < nblocks; n++) {
            if (blocks[n].start->index == k) { print_short_block_info(&blocks[n]); }
        }
    }
}
...
static void test_goemetry_descriptor(geometry_descriptor *gd, int parity) {
    ...
    for (n = 0; n < gd->inner_master_pieces; n++) {
        for (i = gd->master_start[n]; i <= gd->master_end[n]; i++) {
            ...
            if (sites[i].b_type[0] != INNER || sites[i].b_type[1] != INNER || sites[i].b_type[2] != INNER ||
                sites[i].b_type[3] != INNER) {
                lprintf("TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Site %d should be INNER (found in master_piece=%d)\n", i, n);
                print_full_site_info(&sites[i]);
                test_q = false;
            }
            ...
            if (sites[i].test > 1) {
                lprintf("TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Site %d has already appeared in master pieces\n", i);
                print_full_site_info(&sites[i]);
                test_q = false;
            }
        }
    ...
    for (n = 0; n < gd->total_spinor_master_pieces; n++) {
        for (i = gd->master_start[n]; i <= gd->master_end[n]; i++) {
            ...
            if (nlocals < 3) {
                lprintf("TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Site %d should be LOCAL or in a 3D-buffer\n", i);
                print_full_site_info(&sites[i]);
                test_q = false;
            }
        ...
        for (b = blocks; b < blocks + nblocks; b++) {
            if (b->start->index == gd->sbuf_start[n] && b->length == gd->sbuf_len[n] && b->receiver != NULL) {
                ...
                if (this->receiver->test == 1) {
                    ...
                    lprintf(
                        "TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Trying to send block %d to %d, but block %d has already received data!",
                        this->index, this->receiver->index, this->receiver->index);
    ...
    for (n = 0; n < gd->nbuffers_spinor; n++) {
        ...
        for (b = blocks; b < blocks + nblocks; b++) {
            if (b->start->index == gd->rbuf_start[n] && b->length == gd->rbuf_len[n] && b->nsenders != 0) {
                ...
                if (this->test == 1) {
                    lprintf("TEST_GEOMETRY", loglevel, "rbuf[%d] start=%d len=%d\n", n, gd->rbuf_start[n], gd->rbuf_len[n]);
                    lprintf("TEST_GEOMETRY", loglevel,
                            "ERROR test_goemetry_descriptor: Trying to receive block %d, but it has already received data!\n",
                            this->index);
                    print_block_info(this);
                    test_q = false;
                }
        ...
        if (this == NULL) {
            ...
            if (gd->rbuf_from_proc[n] != proc_from) {
                lprintf("TEST_GEOMETRY", loglevel, "rbuf[%d] start=%d len=%d\n", n, gd->rbuf_start[n], gd->rbuf_len[n]);
                lprintf("TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Receive data from processor %d, should be %d\n", gd->rbuf_from_proc[n],
                        proc_from);
                test_q = false;
            }
        }
        ...
        for (n = 0; n < gd->nbuffers_gauge; n++) {
            ...
            if (this == NULL) {
                ...
                if (gd->sbuf_to_proc[n] != proc_to) {
                    lprintf("TEST_GEOMETRY", loglevel, "sbuf[%d] start=%d len=%d\n", n, gd->sbuf_start[n], gd->sbuf_len[n]);
                    lprintf("TEST_GEOMETRY", loglevel,
                            "ERROR test_goemetry_descriptor: Send data to processor %d, should be %d\n", gd->sbuf_to_proc[n],
                            proc_to);
                    test_q = false;
                }
            }
    ...
    for (n = 0; n < gd->ncopies_spinor; n++) {
        for (i = 0; i < gd->copy_len[n]; i++) {
            ...
            if (sites[from].c_type != ORIGINAL || sites[to].c_type != DUPLICATE || sites[to].original != &sites[from]) {
                lprintf(
                    "TEST_GEOMETRY", loglevel,
                    "ERROR test_goemetry_descriptor: Site %d should be ORIGINAL, and site %d should be one of its DUPLICATEs\n",
                    from, to);
    ...
    for (n = 0; n < gd->nbuffers_spinor; n++) {
        for (i = gd->sbuf_start[n]; i < gd->sbuf_start[n] + gd->sbuf_len[n]; i++) {
            if (sites[i].c_type == DUPLICATE && sites[i].test == 0) {
                lprintf("TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Site %d is DUPLICATE, but has not been copied\n", i);
                print_full_site_info(&sites[i]);
                test_q = false;
            }
        }
    ...
    if (gd->gsize_gauge != -1) {
        ...
        for (n = 0; n < gd->ncopies_gauge; n++) {
            memcpy(glb_coord + 4 * (gd->copy_to[n] - gd->master_shift), glb_coord + 4 * (gd->copy_from[n] - gd->master_shift),
                   gd->copy_len[n] * sizeof(int) * 4);
        }
        ...
        for (n = 0; n < gd->gsize_gauge; n++) {
            if (memcmp(glb_coord + 4 * n, sites[n + gd->master_shift].glb_coord, sizeof(int) * 4) != 0) {
                lprintf("TEST_GEOMETRY", loglevel,
                        "ERROR test_goemetry_descriptor: Global coordinates (%d,%d,%d,%d), site %d on neighbour processor\n",
