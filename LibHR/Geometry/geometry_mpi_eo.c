...
#define local_index(nt, nx, ny, nz)                                                                                           \
    (((nt) + 2 * T_BORDER + T) % (2 * T_BORDER + T) + (T + 2 * T_BORDER) * (((nx) + 2 * X_BORDER + X) % (2 * X_BORDER + X)) + \
...
static int init_border(int id_mask, int eotype, int level, int id_zone, int *bl_start, int *incr, int *bl_width,
                       int *match_length);
static void close_border(int i);
static void set_border_pointer(int actualn, int match_p);

static void set_block(int id_mask, int *bl_s, int *incr, int *bl_w, int pts, int wts, int pxs, int wxs, int pys, int wys,
                      int pzs, int wzs) {
...
static void geometry_mpi_init() {
    ...
    BOR_CUBE = 2 * (X * Y * Z * T_BORDER + X * Y * Z_BORDER * T + X * Y_BORDER * Z * T + X_BORDER * Y * Z * T);

    BOR_SQUARE = 4 * (X * Y * Z_BORDER * T_BORDER + X * Y_BORDER * Z * T_BORDER + X * Y_BORDER * Z_BORDER * T +
                      X_BORDER * Y * Z * T_BORDER + X_BORDER * Y * Z_BORDER * T + X_BORDER * Y_BORDER * Z * T);

    TOTAL_VOLUME =
        (X - 2 * X_BORDER) * (Y - 2 * Y_BORDER) * (Z - 2 * Z_BORDER) * (T - 2 * T_BORDER) + 2 * BOR_CUBE + 2 * BOR_SQUARE;
    ...
    error((map_id2overlexi == NULL), 0, "geometry_mpi_init [geometry_mpi.c]", "Cannot allocate memory for map_id2overlexi");

    map_overlexi2id = malloc(OVERSIZE_VOLUME * sizeof(int));
    error((map_overlexi2id == NULL), 0, "geometry_mpi_init [geometry_mpi.c]", "Cannot allocate memory for map_overlexi2id");

    memory = malloc(TOTAL_VOLUME * sizeof(int));
    error((memory == NULL), 0, "geometry_mpi_init [geometry_mpi.c]", "Cannot allocate memory for memory");

    border = malloc((2 + 2 * N_BORDER) * sizeof(border_id));
    error((border == NULL), 1, "geometry_mpi_init [geometry_mpi.c]", "Cannot allocate memory for border");

    if (N_BORDER > 0) {
        glattice.rbuf_len = malloc((N_BORDER) * sizeof(int));
        error((glattice.rbuf_len == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glattice.sbuf_len = malloc((N_BORDER) * sizeof(int));
        error((glattice.sbuf_len == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glattice.sbuf_to_proc = malloc((N_BORDER) * sizeof(int));
        error((glattice.sbuf_to_proc == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glattice.sbuf_start = malloc((N_BORDER) * sizeof(int));
        error((glattice.sbuf_start == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glattice.rbuf_from_proc = malloc((N_BORDER) * sizeof(int));
        error((glattice.rbuf_from_proc == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glattice.rbuf_start = malloc((N_BORDER) * sizeof(int));
        error((glattice.rbuf_start == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");
    }
    ...
    if (L3_BORDER > 0) {
        ...
        error((glat_even.rbuf_len == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.sbuf_len = malloc((L3_BORDER) * sizeof(int));
        error((glat_even.sbuf_len == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.sbuf_to_proc = malloc((L3_BORDER) * sizeof(int));
        error((glat_even.sbuf_to_proc == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.sbuf_start = malloc((L3_BORDER) * sizeof(int));
        error((glat_even.sbuf_start == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.rbuf_from_proc = malloc((L3_BORDER) * sizeof(int));
        error((glat_even.rbuf_from_proc == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.rbuf_start = malloc((L3_BORDER) * sizeof(int));
        error((glat_even.rbuf_start == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.rbuf_len = malloc((L3_BORDER) * sizeof(int));
        error((glat_odd.rbuf_len == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.sbuf_len = malloc((L3_BORDER) * sizeof(int));
        error((glat_odd.sbuf_len == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.sbuf_to_proc = malloc((L3_BORDER) * sizeof(int));
        error((glat_odd.sbuf_to_proc == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.sbuf_start = malloc((L3_BORDER) * sizeof(int));
        error((glat_odd.sbuf_start == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.rbuf_from_proc = malloc((L3_BORDER) * sizeof(int));
        error((glat_odd.rbuf_from_proc == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.rbuf_start = malloc((L3_BORDER) * sizeof(int));
        error((glat_odd.rbuf_start == NULL), 1, "fix_geometry_descriptor [geometry_mpi.c]", "Cannot allocate memory");
    }
    ...
    for (i = 0; i < 2; i++) {
        ...
        error((dir_mask[i] == NULL), 1, "geometry_mpi_init [geometry_mpi.c]", "Cannot allocate memory");
        inv_mask[i] = malloc(4 * sizeof(int));
        error((inv_mask[i] == NULL), 1, "geometry_mpi_init [geometry_mpi.c]", "Cannot allocate memory");
    }
...
void eval_fusemask(geometry_descriptor *lglat) {
    ...
    for (int ip = 0; ip < lglat->local_master_pieces; ip++) {
        ...
        if (ip == lglat->inner_master_pieces) { lglat->fuse_inner_counter = (lglat)->master_start[ip] - (lglat)->master_shift; }
    }
}
...
static void set_inner(int eotype) {
    ...
    set_block(0, bl_start, incr, bl_width, 2 * T_BORDER, T - 2 * T_BORDER, 2 * X_BORDER, X - 2 * X_BORDER, 2 * Y_BORDER,
              Y - 2 * Y_BORDER, 2 * Z_BORDER, Z - 2 * Z_BORDER);
    ...
    index_zone++;
    lprintf("INNER", REPORTLVL, "END info: start%d end %d \n", border[index_border].index_start,
            border[index_border].index_end);
}
...
void walk_on_lattice(int id_mask, int eotype, int level, int id_zone, int *bl_start, int *incr, int *bl_width) {
    ...
    if (level != 4) { evblock[0] = evblock[1] = evblock[2] = evblock[3] = 1; }
    int x0, x1, x2, x3;
    int bx0, bx1, bx2, bx3;
    int x[4];
    int match_length, match_point, steps = 0, last_point;
    lprintf("GEOMETRY", REPORTLVL, "\n\n\nwalk_on_lattice The START (%d,%d,%d,%d) \n", bl_start[dir_mask[id_mask][0]],
            bl_start[dir_mask[id_mask][1]], bl_start[dir_mask[id_mask][2]], bl_start[dir_mask[id_mask][3]]);
    lprintf("GEOMETRY", REPORTLVL, "walk_on_lattice The WIDTH (%d,%d,%d,%d) \n", bl_width[dir_mask[id_mask][0]],
            bl_width[dir_mask[id_mask][1]], bl_width[dir_mask[id_mask][2]], bl_width[dir_mask[id_mask][3]]);
    lprintf("GEOMETRY", REPORTLVL, "walk_on_lattice The incr (%d,%d,%d,%d) \n", incr[dir_mask[id_mask][0]],
            incr[dir_mask[id_mask][1]], incr[dir_mask[id_mask][2]], incr[dir_mask[id_mask][3]]);
    ...
    if (init_border(id_mask, eotype, level, id_zone, bl_start, incr, bl_width, &match_length)) {
        for (bx3 = bl_start[3]; block_cond(bl_start[3], bl_start[3] + bl_width[3], bx3);
             bx3 += incr[3] * evblock[inv_mask[id_mask][3]]) {
            for (bx2 = bl_start[2]; block_cond(bl_start[2], bl_start[2] + bl_width[2], bx2);
                 bx2 += incr[2] * evblock[inv_mask[id_mask][2]]) {
                for (bx1 = bl_start[1]; block_cond(bl_start[1], bl_start[1] + bl_width[1], bx1);
                     bx1 += incr[1] * evblock[inv_mask[id_mask][1]]) {
                    for (bx0 = bl_start[0]; block_cond(bl_start[0], bl_start[0] + bl_width[0], bx0);
                         bx0 += incr[0] * evblock[inv_mask[id_mask][0]]) {
                        for (x3 = 0; (x3 < evblock[inv_mask[id_mask][3]]) &&
                                     block_cond(bl_start[3], bl_start[3] + bl_width[3], bx3 + incr[3] * x3);
                             x3++) {
                            for (x2 = 0; (x2 < evblock[inv_mask[id_mask][2]]) &&
                                         block_cond(bl_start[2], bl_start[2] + bl_width[2], bx2 + incr[2] * x2);
                                 x2++) {
                                for (x1 = 0; (x1 < evblock[inv_mask[id_mask][1]]) &&
                                             block_cond(bl_start[1], bl_start[1] + bl_width[1], bx1 + incr[1] * x1);
                                     x1++) {
                                    for (x0 = 0; (x0 < evblock[inv_mask[id_mask][0]]) &&
                                                 block_cond(bl_start[0], bl_start[0] + bl_width[0], bx0 + incr[0] * x0);
                                         x0++) {
                                        ...
                                        if (eotype == no_eo || eotype == (x[0] + x[1] + x[2] + x[3] + T_BORDER + X_BORDER +
                                                                          Y_BORDER + Z_BORDER + PSIGN) %
                                                                             2) {
...
static int init_border(int id_mask, int eotype, int level, int id_zone, int *bl_start, int *incr, int *bl_width,
                       int *match_length) {
    int i, retval, check_index, match_step, match_interval, length_match = 0, id, found_start, x1, x2, x3, x0, start = 0,
                                                            index_start;
    int x[4];
    int checksum[8];
    int bl_size, foundstart;
    retval = true;
    bl_size = (bl_width[0] * bl_width[1] * bl_width[2] * bl_width[3] > 0) ?
                  bl_width[0] * bl_width[1] * bl_width[2] * bl_width[3] :
                  -bl_width[0] * bl_width[1] * bl_width[2] * bl_width[3];
    border_checksum(checksum, id_mask, bl_start, bl_width);

    for (i = 0; i < index_border + 1; i++) {
        if (border[i].checksum[0] == checksum[0] && border[i].checksum[1] == checksum[1] &&
            border[i].checksum[2] == checksum[2] && border[i].checksum[3] == checksum[3] &&
            border[i].checksum[4] == checksum[4] && border[i].checksum[5] == checksum[5] &&
            border[i].checksum[6] == checksum[6] && border[i].checksum[7] == checksum[7] && border[i].eo_type == eotype) {
            retval = false;
        }
    }
...
static void set_border_l3(int eotype) {
    ...
    int ind[4][2] = { { T_BORDER, T }, { X_BORDER, X }, { Y_BORDER, Y }, { Z_BORDER, Z } };
...
#define reset_tmp            \
    ...
    if (pdir > 0) {
        ...
        set_block(0, bl_start, incr, bl_width, tmp_start[0], tmp_width[0], tmp_start[1], tmp_width[1], tmp_start[2],
                  tmp_width[2], tmp_start[3], tmp_width[3]);

        lprintf("GEOMETRY", REPORTLVL, "set_border_l3 start T[%d] X[%d] Y[%d] Z[%d] \n", bl_start[dir_mask[0][0]],
                bl_start[dir_mask[0][1]], bl_start[dir_mask[0][2]], bl_start[dir_mask[0][3]]);
        lprintf("GEOMETRY", REPORTLVL, "set_border_l3 width T[%d] X[%d] Y[%d] Z[%d] \n", bl_width[dir_mask[0][0]],
                bl_width[dir_mask[0][1]], bl_width[dir_mask[0][2]], bl_width[dir_mask[0][3]]);
        ...
        if (pdir > 2) {
            ...
            set_block(1, bl_start, incr, bl_width, tmp_start[0], tmp_width[0], tmp_start[1], tmp_width[1], tmp_start[2],
                      tmp_width[2], tmp_start[3], tmp_width[3]);

            lprintf("GEOMETRY", REPORTLVL, "set_border_l3 start T[%d] X[%d] Y[%d] Z[%d] \n", bl_start[dir_mask[1][0]],
                    bl_start[dir_mask[1][1]], bl_start[dir_mask[1][2]], bl_start[dir_mask[1][3]]);
            lprintf("GEOMETRY", REPORTLVL, "set_border_l3 width T[%d] X[%d] Y[%d] Z[%d] \n", bl_width[dir_mask[1][0]],
                    bl_width[dir_mask[1][1]], bl_width[dir_mask[1][2]], bl_width[dir_mask[1][3]]);
                            ...
                            for (wd2 = 0; wd2 < 2; wd2++) {
                                ...
                                for (wd3 = 0; wd3 < 2; wd3++) {
                                    ...
                                    if (wd0 + wd1 + wd2 + wd3 == 2) {
                                        set_block(0, bl_start, incr, bl_width, tmp_start[0], tmp_width[0], tmp_start[1],
                                                  tmp_width[1], tmp_start[2], tmp_width[2], tmp_start[3], tmp_width[3]);

                                        lprintf("GEOMETRY", REPORTLVL, "set_border_l2_auto start T[%d] X[%d] Y[%d] Z[%d] \n",
                                                bl_start[dir_mask[0][0]], bl_start[dir_mask[0][1]], bl_start[dir_mask[0][2]],
                                                bl_start[dir_mask[0][3]]);
                                        lprintf("GEOMETRY", REPORTLVL, "set_border_l2_auto width T[%d] X[%d] Y[%d] Z[%d] \n",
                                                bl_width[dir_mask[0][0]], bl_width[dir_mask[0][1]], bl_width[dir_mask[0][2]],
                                                bl_width[dir_mask[0][3]]);
...
static void fix_buffer(int zone_id) {
    ...
    for (i = 0; i < done_border; i++) {
        if (border[i].id_zone == zone_id) {
            ...
            set_block(id_mask, bl_start, incr, bl_width, tmp_start[0], tmp_width[0], tmp_start[1], tmp_width[1], tmp_start[2],
                      tmp_width[2], tmp_start[3], tmp_width[3]);
...
static int correnspondig_buffer(int *bf, int i) {
    ...
    if ((border[i].checksum[0] + border[i].checksum[1] + border[i].checksum[2] + border[i].checksum[3]) % 2 ==
        (checksum[0] + checksum[1] + checksum[2] + checksum[3]) % 2) {
    ...
    for (j = 0; j < done_border; j++) {
        if (border[j].id_zone < 0 && border[j].checksum[0] == checksum[0] && border[j].checksum[1] == checksum[1] &&
            border[j].checksum[2] == checksum[2] && border[j].checksum[3] == checksum[3] &&
            border[j].checksum[4] == checksum[4] && border[j].checksum[5] == checksum[5] &&
            border[j].checksum[6] == checksum[6] && border[j].checksum[7] == checksum[7]) {
            found++;
    ...
    error(found != 2, 1, "correnspondig_buffer [geometry_mpi_eo.c]", "found more than one buffer corrensponding to one border");
...
static void set_memory_order() {
    ...
    if (master_piece_number != 0) {
        ...
        error((glattice.master_start == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glattice.master_end = malloc(master_piece_number * sizeof(unsigned int));
        error((glattice.master_end == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");
    ...
    if (master_piece_number_e != 0) {
        ...
        error((glat_even.master_start == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.master_end = malloc(master_piece_number_e * sizeof(unsigned int));
        error((glat_even.master_end == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");
    ...
    if (master_piece_number_o != 0) {
        ...
        error((glat_odd.master_start == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.master_end = malloc(master_piece_number_o * sizeof(unsigned int));
        error((glat_odd.master_end == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");
    ...
    if (copy_piece_number != 0) {
        ...
        error((glattice.copy_from == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glattice.copy_to = malloc(copy_piece_number * sizeof(int));
        error((glattice.copy_to == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glattice.copy_len = malloc(copy_piece_number * sizeof(int));
        error((glattice.copy_len == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");
    ...
    if (copy_piece_number_e != 0) {
        ...
        error((glat_even.copy_from == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.copy_to = malloc(copy_piece_number_e * sizeof(int));
        error((glat_even.copy_to == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glat_even.copy_len = malloc(copy_piece_number_e * sizeof(int));
        error((glat_even.copy_len == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");
    ...
    if (copy_piece_number_o != 0) {
        ...
        error((glat_odd.copy_from == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.copy_to = malloc(copy_piece_number_o * sizeof(int));
        error((glat_odd.copy_to == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");

        glat_odd.copy_len = malloc(copy_piece_number_o * sizeof(int));
        error((glat_odd.copy_len == NULL), 1, "set_memory_order [geometry_mpi.c]", "Cannot allocate memory");
        ...
        if (border[i].id_zone >= 0 && inner[0] != i && inner[1] != i && border[i].size > 0) {
            ...
            if (border[i].level == 3) {
                if (border[i].eo_type == even) {
                    ...
                    lprintf("GEOMETRY", REPORTLVL, "glattice_eo (%d) E send to %d start %d(%d)", sfbuf_e,
                            (glat_even.sbuf_to_proc)[sfbuf_e], (glat_even.sbuf_start)[sfbuf_e], (glat_even.sbuf_len)[sfbuf_e]);
                    ...
                    lprintf("GEOMETRY", REPORTLVL, " -> E receive from %d start %d(%d) \n", (glat_even.rbuf_from_proc)[sfbuf_e],
                            (glat_even.rbuf_start)[sfbuf_e], (glat_even.rbuf_len)[sfbuf_e]);
                }
                ...
                if (border[i].eo_type == odd) {
                    ...
                    lprintf("GEOMETRY", REPORTLVL, "glattice_eo (%d) O send to %d start %d(%d)", sfbuf_o,
                            (glat_odd.sbuf_to_proc)[sfbuf_o], (glat_odd.sbuf_start)[sfbuf_o], (glat_odd.sbuf_len)[sfbuf_o]);
                    ...
                    lprintf("GEOMETRY", REPORTLVL, " -> O receive from %d start %d(%d) \n", (glat_odd.rbuf_from_proc)[sfbuf_o],
                            (glat_odd.rbuf_start)[sfbuf_o], (glat_odd.rbuf_len)[sfbuf_o]);
                }
            }
...
void geometry_mpi_eo(void) {
    ...
    atexit(&free_geometry_mpi_eo);
}

void free_geometry_mpi_eo() {
    if (glattice.master_start != NULL) { free(glattice.master_start); }
    if (glattice.master_end != NULL) { free(glattice.master_end); }
    if (glattice.copy_from != NULL) { free(glattice.copy_from); }
    if (glattice.copy_to != NULL) { free(glattice.copy_to); }
    if (glattice.copy_len != NULL) { free(glattice.copy_len); }
    if (glattice.rbuf_len != NULL) { free(glattice.rbuf_len); }
    if (glattice.sbuf_len != NULL) { free(glattice.sbuf_len); }
    if (glattice.rbuf_from_proc != NULL) { free(glattice.rbuf_from_proc); }
    if (glattice.rbuf_start != NULL) { free(glattice.rbuf_start); }
    if (glattice.sbuf_to_proc != NULL) { free(glattice.sbuf_to_proc); }
    if (glattice.sbuf_start != NULL) { free(glattice.sbuf_start); }

    if (glat_even.master_start != NULL) { free(glat_even.master_start); }
    if (glat_even.master_end != NULL) { free(glat_even.master_end); }
    if (glat_even.copy_from != NULL) { free(glat_even.copy_from); }
    if (glat_even.copy_to != NULL) { free(glat_even.copy_to); }
    if (glat_even.copy_len != NULL) { free(glat_even.copy_len); }
    if (glat_even.rbuf_len != NULL) { free(glat_even.rbuf_len); }
    if (glat_even.sbuf_len != NULL) { free(glat_even.sbuf_len); }
    if (glat_even.rbuf_from_proc != NULL) { free(glat_even.rbuf_from_proc); }
    if (glat_even.rbuf_start != NULL) { free(glat_even.rbuf_start); }
    if (glat_even.sbuf_to_proc != NULL) { free(glat_even.sbuf_to_proc); }
    if (glat_even.sbuf_start != NULL) { free(glat_even.sbuf_start); }

    if (glat_odd.master_start != NULL) { free(glat_odd.master_start); }
    if (glat_odd.master_end != NULL) { free(glat_odd.master_end); }
    if (glat_odd.copy_from != NULL) { free(glat_odd.copy_from); }
    if (glat_odd.copy_to != NULL) { free(glat_odd.copy_to); }
    if (glat_odd.copy_len != NULL) { free(glat_odd.copy_len); }
    if (glat_odd.rbuf_len != NULL) { free(glat_odd.rbuf_len); }
    if (glat_odd.sbuf_len != NULL) { free(glat_odd.sbuf_len); }
    if (glat_odd.rbuf_from_proc != NULL) { free(glat_odd.rbuf_from_proc); }
    if (glat_odd.rbuf_start != NULL) { free(glat_odd.rbuf_start); }
    if (glat_odd.sbuf_to_proc != NULL) { free(glat_odd.sbuf_to_proc); }
    if (glat_odd.sbuf_start != NULL) { free(glat_odd.sbuf_start); }
}
...
void geometry_mem_alloc() {
    ...
    if (init) {
        ...
        error((alloc_mem == NULL), 1, "geometry_init [geometry_init.c]", "Cannot allocate memory");
