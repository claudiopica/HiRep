...
static void sync_spinor_field_force(spinor_field_force *p) {
  int i;
  /* int j, x, y; */
  geometry_descriptor *gd = p->type;
...
void complete_sf_force_sendrecv(spinor_field_force *sf) {
#ifdef WITH_MPI
  int mpiret; (void)mpiret; // Remove warning of variable set but not used
  int nreq=2*sf->type->nbuffers_gauge;
  ...
  if(nreq>0) {
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      error(1,1,"complete_sf_force_sendrecv " __FILE__,"Cannot complete communications");
    }
#endif
  }
	...
	gettimeofday(&sfend,0);
	timeval_subtract(&sfetime,&sfend,&sfstart);
	lprintf("MPI TIMING",0,"complete_sf_force_sendrecv" __FILE__ " %ld sec %ld usec\n",sfetime.tv_sec,sfetime.tv_usec);
	sf_control=0;
      }
#endif
...
void start_sf_force_sendrecv(spinor_field_force *sf) {
#ifdef WITH_MPI
  int i, mpiret; (void)mpiret; // Remove warning of variable set but not used
  geometry_descriptor *gd=sf->type;
  ...
  /* questo credo che non sia il modo piu' efficiente!!! */
  /* bisognerebbe forse avere una variabile di stato nei campi?? */
  complete_sf_force_sendrecv(sf);

  /* fill send buffers */
  sync_spinor_field_force(sf);
#ifdef MPI_TIMING
  error(sf_control>0,1,"start_sf_force_sendrecv " __FILE__,"Multiple send without receive");
  gettimeofday(&sfstart,0);  
  sf_control=1;
#endif
  ...
  for (i=0; i<(gd->nbuffers_gauge); ++i) {
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      MPI_Error_string(mpiret,mesg,&mesglen);
      lprintf("MPI",0,"ERROR: %s\n",mesg);
      error(1,1,"start_sf_force_sendrecv " __FILE__,"Cannot start send buffer");
    }
#endif
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      MPI_Error_string(mpiret,mesg,&mesglen);
      lprintf("MPI",0,"ERROR: %s\n",mesg);
      error(1,1,"start_sf_force_sendrecv " __FILE__,"Cannot start receive buffer");
    }
#endif
