...
static void _propagator_baryon1_mul2(hr_complex C[4][4], suNf_propagator S, suNf_propagator Stilde, int i, int ip, int j,
                                     int jp, int k, int kp) {
    hr_complex tmp[4][4];
...
static void _propagator_baryon2_mul2(hr_complex C[4][4], suNf_propagator S, suNf_propagator Stilde, int i, int ip, int j,
                                     int jp, int k, int kp) {
    hr_complex tmp;
    _complex_0(tmp);
                        ...
                        for (int ap = 0; ap < NF; ++ap) {
                            for (int b = 0; b < NF; ++b) {
                                for (int bp = 0; bp < NF; ++bp) {
                                    for (int c = 0; c < NF; ++c) {
                                        for (int cp = 0; cp < NF; ++cp) {
                                            if (fabs(col_factor[a][b][c][ap][bp][cp]) < 1e-5) { continue; }

                                            // Nucleon
                                            for (int k = 0; k < 2; k++) {
                                                for (int l = 0; l < 2; l++) {
                                                    _propagator_baryon1_mul2(C1, S, Snucleon[k][l], c, cp, b, bp, a,
                                                                             ap); // nucleon term 1
                                                    _propagator_baryon2_mul2(C2, S, Snucleon[k][l], c, ap, a, cp, b,
                                                                             bp); // nucleon term 2
                                                    ...
                                                    for (int i = 0; i < 4; i++) {
                                                        for (int j = 0; j < 4; j++) {
                                                            corr_nucleon[tc][k][l][i][j] -=
                                                                col_factor[a][b][c][ap][bp][cp] * (C1[i][j] - C2[i][j]);
                                                        }
                                                    }
                                                }
                                            }
                                            ...
                                            for (int mu = 0; mu < 4; mu++) {
                                                for (int nu = 0; nu < 4; nu++) {
                                                    _propagator_baryon1_mul2(C1, S, Sdelta[mu][nu], c, cp, b, bp, a,
                                                                             ap); // delta term 1
                                                    _propagator_baryon2_mul2(C2, S, Sdelta[mu][nu], c, ap, a, cp, b,
                                                                             bp); // delta term 2

                                                    // Multiply by color factor and accumulate in the correlator
                                                    for (int i = 0; i < 4; i++) {
                                                        for (int j = 0; j < 4; j++) {
                                                            corr_delta[tc][mu][nu][i][j] -=
                                                                2 * col_factor[a][b][c][ap][bp][cp] * (C2[i][j] - 2 * C1[i][j]);
                                                        }
                                                    }
                                                }
                                            }
            ...
            for (int l = 0; l < 2; l++) {
                for (int i = 0; i < 4; i++) {
                    lprintf("CORR_NUCLEON", 0, "%d %d %d %3.10e %3.10e  %3.10e %3.10e  %3.10e %3.10e  %3.10e %3.10e \n", t, k,
                            l, creal(corr_nucleon[t][k][l][i][0]), cimag(corr_nucleon[t][k][l][i][0]),
            ...
            for (int nu = 0; nu < 4; nu++) {
                for (int i = 0; i < 4; i++) {
                    lprintf("CORR_DELTA", 0, "%d %d %d %3.10e %3.10e  %3.10e %3.10e  %3.10e %3.10e  %3.10e %3.10e \n", t, mu,
                            nu, creal(corr_delta[t][mu][nu][i][0]), cimag(corr_delta[t][mu][nu][i][0]),
