...
void polyakov()
{
  ...
  for (int mu = 0; mu < 4; mu++)
  {
    ...
    {
      MPI_Status status;
      MPIRET(mpiret) MPI_Recv(bp,                                     /* buffer */
    ...
    if (COORD[mu] != np[mu] - 1)
    {
      MPIRET(mpiret) MPI_Send(p,                                      /* buffer */
                        size3d * sizeof(suNg) / sizeof(double), /* lenght in units of doubles */
                        MPI_DOUBLE,                             /* basic datatype */
    ...
    if (COORD[mu] == np[mu] - 1)
    {
      ...
      for (int t = 0; t < np[mu] - 1; t++)
      {
        ...
        MPIRET(mpiret) MPI_Isend(p,                                      /* buffer */
                           size3d * sizeof(suNg) / sizeof(double), /* lenght in units of doubles */
                           MPI_DOUBLE,                             /* basic datatype */
      ...
      MPIRET(mpiret) MPI_Waitall(np[mu] - 1, comm_req, status);
#ifndef NDEBUG
      if (mpiret != MPI_SUCCESS)
      {
        char mesg[MPI_MAX_ERROR_STRING];
...
#endif /* NDEBUG */
      MPI_Status status;
      MPIRET(mpiret) MPI_Recv(p,                                      /* buffer */
                        size3d * sizeof(suNg) / sizeof(double), /* lenght in units of doubles */
                        MPI_DOUBLE,                             /* basic datatype */
