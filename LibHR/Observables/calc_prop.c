...
//static spinor_field *tmp_odd2 = NULL;
...
static void calc_propagator_eo_tw_core(spinor_field *psi, spinor_field *eta, int solver)
{
  ...
  spinor_field_copy_f(tmp_odd, &qprop_mask_eta);
  Dxx_tw_inv(tmp_odd, mass[0], tw_mass, DIRECT);
  ...
  {
    /*This should be for the multishift case please check " 
    
    ANTONIO!!!! we stoped here*/

    psi[0].type = &glat_even;
    spinor_field_copy_f(resd, psi);
    Dxx_tw_inv(resd, mass[0], tw_mass, DAGGER); // PRUEBA HERM
    psi[0].type = &glattice;
  ...
  if (solver == _CG)
  {
    ...
    spinor_field_copy_f(&qprop_mask_eta, tmp);
    spinor_field_g5_assign_f(&qprop_mask_eta);
    Q_pre_dag(tmp, &qprop_mask_eta);
    cgiter += cg_mshift(&QMR_par, &Q2_pre, tmp, resd);
  }
  else if (solver == _MINRES)
  {
    spinor_field_g5_f(tmp, tmp);
    cgiter += MINRES_mshift(&QMR_par, &Q_pre, tmp, resd);
  }
  else
  {
    //    cgiter+=g5QMR_mshift(&QMR_par, &D_pre, tmp, resd);
  ...
  /* compute solution
     psi_even = D_ee*resd_e
     psi_odd = D_oo^-1*eta_odd-Doo^-1 D_oe Dee^-1 resd_e
  */

  qprop_mask_psi = *psi;
  qprop_mask_psi.type = &glat_even;
  //  spinor_field_copy_f(tmp_norm,resd);
  Dxx_tw_inv(resd, mass[0], tw_mass, -1);
  spinor_field_mul_f(resd, ((4. + mass[0]) * (4. + mass[0]) + tw_mass * tw_mass), resd);
  ...
  // Dphi_(&qprop_mask_psi,resd);
  Dphi_(auxodd, resd);
  m4g5(auxodd, mass[0], tw_mass, -1);

  spinor_field_sub_f(&qprop_mask_psi, tmp_odd, auxodd);

  ++cgiter; /* One whole call*/
  lprintf("CALC_PROP_CORE", 0, "QMR_eo MVM = %d\n", cgiter);

  start_sf_sendrecv(psi);
  complete_sf_sendrecv(psi);
}

void calc_propagator_tw(spinor_field *psi, spinor_field *eta, int ndilute)
{
  int beta, i, n_masses;
  double *m;
  m = mass;
...
#ifdef WITH_CLOVER
    lprintf("CALC_PROPAGATOR_TW",0,"Calc propagator has no csw improved implementation. The TM Dirac Operator is automatically O(a^2) improved ");
#endif
#ifdef GAUSSIAN_NOISE
  ...
  //if (tmp_odd2 == NULL)
  //{
  //  tmp_odd2 = alloc_spinor_field_f(1, &glat_odd);
  //  tmp_norm = alloc_spinor_field_f(1, &glat_even);
  //}

  error(init == 0, 1, "calc_prop.c", "calc_propagator_tw method not initialized!");
