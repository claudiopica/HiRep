...
chisf_mem *init_rotated_corr_mem() {
    ...
    corrmem->g1 = malloc(16 * GLB_T * sizeof(hr_complex));
    ...
    corrmem->g1_ij = malloc(13 * GLB_T * sizeof(hr_complex **));
    ...
    hr_complex **tmp = malloc(GLB_T * 13 * 4 * NF * sizeof(hr_complex *));
    hr_complex *tmp1 = malloc(4 * NF * GLB_T * 13 * 4 * NF * sizeof(hr_complex));
    ...
    (corrmem->l13_ij)[GLB_T - 1][4 * NF - 1][4 * NF - 1] = 1. + 0. * I;
    corrmem->M = malloc(4 * NF * sizeof(hr_complex *));
    for (j = 0; j < 4 * NF; j++) {
        corrmem->M[j] = malloc(4 * NF * sizeof(hr_complex));
    }
...
gvuu+ = -  1/2 sum  csi^dag U0(1,z) Huu^(-1)(2,z;x)       g5g0      Huu^(-1)(x:2,y) U0(1,y)^dag psi      psi^dag g0 Q- csi
  **************************************************/
    hr_complex ***g1_ij = corr_mem->g1_ij, ***g2_ij = corr_mem->g2_ij, ***g3_ij = corr_mem->g3_ij, ***g4_ij = corr_mem->g4_ij,
               *g1 = corr_mem->g1, *g2 = corr_mem->g2, *g3 = corr_mem->g3, *g4 = corr_mem->g4, **M = corr_mem->M;
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                _complex_mulr(g4_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], .5 / GLB_VOL3,
                              g4_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(g2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              g2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(g1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              g1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(g3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              g3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                // Normalization with 0.5/GLB_VOL3
                _complex_mulr(g4_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              g4_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(g2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 0.5 / GLB_VOL3,
                              g2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(g1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 0.5 / GLB_VOL3,
...
void rotated_gvtuup(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex ***g1_ij = corr_mem->g1_ij, ***g2_ij = corr_mem->g2_ij, *g1 = corr_mem->g1, *g2 = corr_mem->g2,
               *g3 = corr_mem->g3, **M = corr_mem->M;
...
void rotated_g1uup(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex ***g1_ij = corr_mem->g1_ij, *g1 = corr_mem->g1, **M = corr_mem->M;
