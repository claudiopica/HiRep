...
static int g5QMR_mshift_core(short *valid, mshift_par *par, int trunc_iter, spinor_operator M, spinor_field *in, spinor_field *out_trunc, spinor_field *out)
{
  ...
  */
  ...
  if (par->n == 1)
  { /* in this case is not a multishift and we use as starting vector out[0] */
    M(Mp, &out[0]);
  ...
  for (i = 0; i < (par->n); ++i)
  {
    ...
    s1[i] = s2[i] = 0.;
    if (par->n != 1)
      spinor_field_zero_f(&out[i]); /* if no multishift we start with the trial solution */
  ...
  do
  {
    ...
    if (fabs(rho) > 1.e-13)
    {
      ...
      if (fabs(delta) < 1.e-6 || maxm < 1.e-3)
