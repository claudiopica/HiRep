...
void measure_loops(double *m, int nhits, int conf_num, double precision, int source_type, int n_mom, storage_switch swc, data_storage_array **ret)
{
	...
	if (source_type == 0)
		lprintf("CORR", 0, "Pure volume source  will be used  \n");
	if (source_type == 1)
		lprintf("CORR", 0, "Gauge fixed source  with time and spin dilution will be used \n");
	if (source_type == 2)
		lprintf("CORR", 0, "Time and spin dilution  will be used \n");
	if (source_type == 3)
		lprintf("CORR", 0, "Time, spin and color dilution  will be used \n");
	if (source_type == 4)
