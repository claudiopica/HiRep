            ...
            for (pz = -n_mom; pz <= n_mom; ++pz) {
                    ...
                    for (x = 0; x < X; x++) {
                        for (y = 0; y < Y; y++) {
                            for (z = 0; z < Z; z++) {
                                ...
                                pdotx = 2.0 * PI *
                                        (((double)(p_tot_x - px)) * (x + zerocoord[1]) / GLB_X +
                                         ((double)(p_tot_y - py)) * (y + zerocoord[2]) / GLB_Y +
                                         ((double)(p_tot_z - pz)) * (z + zerocoord[3]) / GLB_Z);
                                cpdotx = cos(pdotx);
                                spdotx = sin(pdotx);
                ...
                for (t = 0; t < GLB_T; t++) {
                    ...
                    while (motmp != NULL) {
                        ...
                        motmp->corr_re[INDEX(px, py, pz, n_mom, tc)] =
                            (tr1re[tc] * tr2re[tc] - tr1im[tc] * tr2im[tc]); //(GLB_VOL3*GLB_VOL3);
                        motmp->corr_im[INDEX(px, py, pz, n_mom, tc)] =
                            (tr1re[tc] * tr2im[tc] + tr2re[tc] * tr1im[tc]); //(GLB_VOL3*GLB_VOL3);
                        motmp = motmp->next;
                    }
                }
            }
                    ...
                    for (x = 0; x < X; x++) {
                        for (y = 0; y < Y; y++) {
                            for (z = 0; z < Z; z++) {
                                ...
                                pdotx = 2.0 * PI *
                                        (((double)p_tot_x - (double)px) * (x + zerocoord[1]) / GLB_X +
                                         ((double)p_tot_y - (double)py) * (y + zerocoord[2]) / GLB_Y +
                                         ((double)p_tot_z - (double)pz) * (z + zerocoord[3]) / GLB_Z);
                                phase = cos(pdotx) - I * sin(pdotx);
                                for (mu = 0; mu < 4; mu++) {
                                    for (gamma = 0; gamma < 4; gamma++) {
