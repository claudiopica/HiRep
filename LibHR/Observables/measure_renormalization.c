...
char *tr_channel_names[NCHANNELSR] = { "Sin",    "Sout",   "g5",   "id",   "g0",   "g1",   "g2",   "g3",
                                       "g0g1",   "g0g2",   "g0g3", "g5g0", "g5g1", "g5g2", "g5g3", "g5g0g1",
                                       "g5g0g2", "g5g0g3", "cg0",  "cg1",  "cg2",  "cg3" };
            ...
            for (x = 0; x < X; x++) {
                for (y = 0; y < Y; y++) {
                    for (z = 0; z < Z; z++) {
                        ix = ipt(t, x, y, z);
                        pinx = 2.0 * PI *
                               (((double)pt_in) * (t + zerocoord[0]) / GLB_T + ((double)px_in) * (x + zerocoord[1]) / GLB_X +
                                ((double)py_in) * (y + zerocoord[2]) / GLB_Y + ((double)pz_in) * (z + zerocoord[3]) / GLB_Z);
                        poutx = 2.0 * PI *
                                (((double)pt_out) * (t + zerocoord[0]) / GLB_T + ((double)px_out) * (x + zerocoord[1]) / GLB_X +
                                 ((double)py_out) * (y + zerocoord[2]) / GLB_Y + ((double)pz_out) * (z + zerocoord[3]) / GLB_Z);
...
static void print_renormalization_core(int channel, int conf, int nm, double *mass, char *label, int pt_in, int px_in,
                                       int py_in, int pz_in, int pt_out, int px_out, int py_out, int pz_out) {
    int i, a, b, alpha, beta;

    for (i = 0; i < nm; i++) {
        lprintf("MAIN", 0, "conf #%d mass=%2.6f %s %s p_in(%d,%d,%d,%d) p_out(%d,%d,%d,%d) =", conf, mass[i], label,
                tr_channel_names[channel], pt_in, px_in, py_in, pz_in, pt_out, px_out, py_out, pz_out);
    ...
    if (init != 0) {
        for (i = 0; i < nm; i++) {
            for (k = 0; k < NCHANNELSR; k++) {
                ...
                print_renormalization_core(k, conf, nm, mass, label, pt_in, px_in, py_in, pz_in, pt_out, px_out, py_out,
                                           pz_out);
            }
        }
    }
