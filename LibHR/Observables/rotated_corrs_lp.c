...
void rotated_lXuup(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *lauu_p = corr_mem->g1, *lvuu_p = corr_mem->g2, *ltuu_p = corr_mem->g3, *lttuu_p = corr_mem->g4,
               **M = corr_mem->M;

    hr_complex *lauu_1p = corr_mem->l11, *lvuu_1p = corr_mem->l21, *ltuu_1p = corr_mem->l31, *lttuu_1p = corr_mem->l41;
    hr_complex *lauu_2p = corr_mem->l12, *lvuu_2p = corr_mem->l22, *ltuu_2p = corr_mem->l32, *lttuu_2p = corr_mem->l42;
    hr_complex *lauu_3p = corr_mem->l13, *lvuu_3p = corr_mem->l23, *ltuu_3p = corr_mem->l33, *lttuu_3p = corr_mem->l43;
...
lttuu+ = - i 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Huu^(-1)(2,z;x)    sig0k     Huu^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag g5 gk Q- csi
  ...
  **************************************************/
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                _complex_mulr(la1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 1.0 / (6.0 * GLB_VOL3),
                              la1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(lv1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              lv1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(lt1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              lt1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(ltt1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              ltt1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                _complex_mulr(la2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              la2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(lv2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 1.0 / (6.0 * GLB_VOL3),
                              lv2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(lt2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 1.0 / (6.0 * GLB_VOL3),
                              lt2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(ltt2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 1.0 / (6.0 * GLB_VOL3),
                              ltt2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                // Normalization with 1.0/(6.0*GLB_VOL3), (Remember that the minus signs pf lY1 and lY3 cancels out from the imaginary part of the gamma matrix
                _complex_mulr(la3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 1.0 / (6.0 * GLB_VOL3),
                              la3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(lv3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              lv3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(lt3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              lt3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(ltt3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -1.0 / (6.0 * GLB_VOL3),
                              ltt3_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(lauu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lauu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lauu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lauu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lauu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lauu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lvuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvuu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvuu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvuu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(ltuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltuu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltuu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltuu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lttuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttuu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttuu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttuu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttuu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
    ...
    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " la_uu_+[%d] = %.10e,%.10e\n", ix0, creal(lauu_p[ix0]), cimag(lauu_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " lv_uu_+[%d] = %.10e,%.10e\n", ix0, creal(lvuu_p[ix0]), cimag(lvuu_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " lt_uu_+[%d] = %.10e,%.10e\n", ix0, creal(ltuu_p[ix0]), cimag(ltuu_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " ltt_uu_+[%d] = %.10e,%.10e\n", ix0, creal(lttuu_p[ix0]), cimag(lttuu_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");
}

void rotated_lXddp(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *ladd_p = corr_mem->g1, *lvdd_p = corr_mem->g2, *ltdd_p = corr_mem->g3, *lttdd_p = corr_mem->g4,
               **M = corr_mem->M;

    hr_complex *ladd_1p = corr_mem->l11, *lvdd_1p = corr_mem->l21, *ltdd_1p = corr_mem->l31, *lttdd_1p = corr_mem->l41;
    hr_complex *ladd_2p = corr_mem->l12, *lvdd_2p = corr_mem->l22, *ltdd_2p = corr_mem->l32, *lttdd_2p = corr_mem->l42;
    hr_complex *ladd_3p = corr_mem->l13, *lvdd_3p = corr_mem->l23, *ltdd_3p = corr_mem->l33, *lttdd_3p = corr_mem->l43;
...
lttdd+ = i 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Hdd^(-1)(2,z;x)    sig0k     Hdd^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag g5 gk Q+ csi
  ...
  **************************************************/
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(ladd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladd_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladd_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladd_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lvdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdd_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdd_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdd_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(ltdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdd_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdd_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdd_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lttdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdd_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdd_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdd_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdd_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
    ...
    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " la_dd_+[%d] = %.10e,%.10e\n", ix0, creal(ladd_p[ix0]), cimag(ladd_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " lv_dd_+[%d] = %.10e,%.10e\n", ix0, creal(lvdd_p[ix0]), cimag(lvdd_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " lt_dd_+[%d] = %.10e,%.10e\n", ix0, creal(ltdd_p[ix0]), cimag(ltdd_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " ltt_dd_+[%d] = %.10e,%.10e\n", ix0, creal(lttdd_p[ix0]), cimag(lttdd_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");
}

void rotated_lXudp(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *laud_p = corr_mem->g1, *lvud_p = corr_mem->g2, *ltud_p = corr_mem->g3, *lttud_p = corr_mem->g4,
               **M = corr_mem->M;

    hr_complex *laud_1p = corr_mem->l11, *lvud_1p = corr_mem->l21, *ltud_1p = corr_mem->l31, *lttud_1p = corr_mem->l41;
    hr_complex *laud_2p = corr_mem->l12, *lvud_2p = corr_mem->l22, *ltud_2p = corr_mem->l32, *lttud_2p = corr_mem->l42;
    hr_complex *laud_3p = corr_mem->l13, *lvud_3p = corr_mem->l23, *ltud_3p = corr_mem->l33, *lttud_3p = corr_mem->l43;
...
lttud+ = - 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Huu^(-1)(2,z;x)    sig0k     Hdd^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag gk Q- csi
  ...
  **************************************************/
    ...
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(laud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            laud_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(laud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            laud_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(laud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            laud_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, laud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        laud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lvud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvud_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvud_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvud_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, lvud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        lvud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(ltud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltud_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltud_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltud_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, ltud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        ltud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lttud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttud_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttud_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttud_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttud_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    ...
    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " la_ud_+[%d] = %.10e,%.10e\n", ix0, creal(laud_p[ix0]), cimag(laud_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " lv_ud_+[%d] = %.10e,%.10e\n", ix0, creal(lvud_p[ix0]), cimag(lvud_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " lt_ud_+[%d] = %.10e,%.10e\n", ix0, creal(ltud_p[ix0]), cimag(ltud_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");

    for (ix0 = 0; ix0 < GLB_T - 1; ix0++) {
        lprintf("PC_twisted_AC", 10, " ltt_ud_+[%d] = %.10e,%.10e\n", ix0, creal(lttud_p[ix0]), cimag(lttud_p[ix0]));
    }
    lprintf("PC_twisted_AC", 10, "\n");
}

void rotated_lXdup(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *ladu_p = corr_mem->g1, *lvdu_p = corr_mem->g2, *ltdu_p = corr_mem->g3, *lttdu_p = corr_mem->g4,
               **M = corr_mem->M;

    hr_complex *ladu_1p = corr_mem->l11, *lvdu_1p = corr_mem->l21, *ltdu_1p = corr_mem->l31, *lttdu_1p = corr_mem->l41;
    hr_complex *ladu_2p = corr_mem->l12, *lvdu_2p = corr_mem->l22, *ltdu_2p = corr_mem->l32, *lttdu_2p = corr_mem->l42;
    hr_complex *ladu_3p = corr_mem->l13, *lvdu_3p = corr_mem->l23, *ltdu_3p = corr_mem->l33, *lttdu_3p = corr_mem->l43;
...
lttdu+ = 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Hdd^(-1)(2,z;x)    sig0k     Huu^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag gk Q+ csi
  ...
  **************************************************/
    ...
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(ladu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, ladu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        ladu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lvdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, lvdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        lvdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(ltdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, ltdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        ltdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lttdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdu_1p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdu_2p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdu_p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdu_3p[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
