...
void rotated_lXuum(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *lauu_m = corr_mem->g1, *lvuu_m = corr_mem->g2, *ltuu_m = corr_mem->g3, *lttuu_m = corr_mem->g4,
               **M = corr_mem->M;
...
lttuu- = - i 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Huu^(-1)(2,z;x)    sig0k     Huu^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag g5 gk Q+ csi
  ...
  **************************************************/
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(lauu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lauu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lauu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lauu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lauu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lauu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lvuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvuu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvuu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvuu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(ltuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltuu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltuu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltuu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lttuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttuu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttuu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttuu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
...
void rotated_lXddm(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *ladd_m = corr_mem->g1, *lvdd_m = corr_mem->g2, *ltdd_m = corr_mem->g3, *lttdd_m = corr_mem->g4,
               **M = corr_mem->M;
  ...
  lttdd- = i 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Hdd^(-1)(2,z;x)    sig0k     Hdd^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag g5 gk Q- csi
    ...
    **************************************************/
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(ladd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladd_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladd_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladd_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lvdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdd_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdd_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdd_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(ltdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdd_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdd_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdd_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);

        _complex_add_assign(lttdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdd_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdd_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdd_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
...
lttud- = - 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Huu^(-1)(2,z;x)    sig0k     Hdd^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag gk Q+ csi
  ...
  **************************************************/
    ...
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(laud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            laud_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(laud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            laud_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(laud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            laud_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, laud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        laud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lvud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvud_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvud_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvud_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, lvud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        lvud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(ltud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltud_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltud_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltud_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, ltud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        ltud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lttud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttud_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttud_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttud_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
...
lttdu- = 1/6 sum_k=1^3 sum tr csi^dag U0(1,z) Hdd^(-1)(2,z;x)    sig0k     Huu^(-1)(x:2,y) U0(1,y)^dag psi       psi^dag gk Q- csi
  ...
  **************************************************/
    ...
    for (ix0 = 0; ix0 < T; ix0++) {
        _complex_add_assign(ladu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ladu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ladu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, ladu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        ladu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lvdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lvdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lvdu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, lvdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        lvdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(ltdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(ltdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            ltdu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_mul(temp_comp2, temp_comp, ltdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        ltdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T] = temp_comp2;

        _complex_add_assign(lttdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdu_1m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdu_2m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(lttdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            lttdu_3m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
