...
{
    *op_out += +(2.) * mom_def_Im_tr_paths[2] + (2.) * mom_def_Im_tr_paths[3] + (2.) * mom_def_Im_tr_paths[4] + (2.) * mom_def_Im_tr_paths[5];
}
...
{
    *op_out += +mom_def_Im_tr_paths[8] + mom_def_Im_tr_paths[9] + mom_def_Im_tr_paths[10] + mom_def_Im_tr_paths[11] + mom_def_Im_tr_paths[12] + mom_def_Im_tr_paths[13] + mom_def_Im_tr_paths[14] + mom_def_Im_tr_paths[15];
}
...
{
    *op_out += +(2.) * mom_def_Re_tr_paths[2] + (2.) * mom_def_Re_tr_paths[3] + (2.) * mom_def_Re_tr_paths[4] + (2.) * mom_def_Re_tr_paths[5];
}
...
{
    *op_out += +mom_def_Re_tr_paths[8] + mom_def_Re_tr_paths[9] + mom_def_Re_tr_paths[10] + mom_def_Re_tr_paths[11] + mom_def_Re_tr_paths[12] + mom_def_Re_tr_paths[13] + mom_def_Re_tr_paths[14] + mom_def_Re_tr_paths[15];
}

static double complex c0;
static void OP_oneTr_p_1_1_0_Ir_1_C_1_n_1(double complex *op_out)
{
    *op_out += +(4.) * c0 * mom_def_Re_tr_paths[0];
}
...
void eval_all_glueball_ops(int t, double complex *numerical_op)
{
    ...
    OP_oneTr_p_1_1_0_Ir_1_C_1_n_1(numerical_op + 4);
}
...
void evalute_correlators(cor_list *lcor, int nblocking, double complex *gb_storage, double complex *cor_storage)
{
            ...
            for (int b1 = 0; b1 < basesize; b1++)
            {
                ...
                for (int n2 = 0; n2 < nblocking; n2++)
                    for (int b2 = 0; b2 < basesize; b2++)
                    {
                        ...
                        //lprintf("test",0,"%lf %lf\n",creal(gb1[i1]),cimag(gb1[i1]));

                        tmp = norm * (conj(gb1[i1]) * gb2[i2] + conj(gb1[i2]) * gb2[i1]);
                        //lprintf("test2",0,"%lf %lf\n",creal(tmp),cimag(tmp));
    ...
    totalsize = 0;
    basesize = 4;
    for (int icor = 0 + totalsize; icor < (nblocking * nblocking * basesize * basesize * lcor->n_corrs) + totalsize; icor++)
        lprintf("output ML", 0, "corr %.10e %.10e \n", creal(cor_storage[icor]), cimag(cor_storage[icor]));
    totalsize += (nblocking * nblocking * basesize * basesize * (lcor->n_corrs));
