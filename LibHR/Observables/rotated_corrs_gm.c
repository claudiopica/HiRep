...
void rotated_gXuum(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *gsuu_m = corr_mem->g1, *gpuu_m = corr_mem->g2, *gvuu_m = corr_mem->g3, *gauu_m = corr_mem->g4,
               **M = corr_mem->M;
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                _complex_mulr(ga_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], .5 / GLB_VOL3,
                              ga_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(gp_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              gp_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(gs_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              gs_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(gv_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              gv_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
...
void rotated_gXddm(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *gsdd_m = corr_mem->g1, *gpdd_m = corr_mem->g2, *gvdd_m = corr_mem->g3, *gadd_m = corr_mem->g4,
               **M = corr_mem->M;
...
void rotated_gXudm(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *gsud_m = corr_mem->g1, *gpud_m = corr_mem->g2, *gvud_m = corr_mem->g3, *gaud_m = corr_mem->g4,
               **M = corr_mem->M;
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T; ix0++) {
                ...
                // Normalization with 0.5/GLB_VOL3
                _complex_mulr(ga_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              ga_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(gp_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 0.5 / GLB_VOL3,
                              gp_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
                _complex_mulr(gs_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], 0.5 / GLB_VOL3,
...
void rotated_gXdum(chisf_mem *corr_mem, suNf_spinor *chi, spinor_field *prop_uu, spinor_field *prop_dd) {
    hr_complex *gsdu_m = corr_mem->g1, *gpdu_m = corr_mem->g2, *gvdu_m = corr_mem->g3, *gadu_m = corr_mem->g4,
               **M = corr_mem->M;
...
gvtuu- = - gvt1uu- + gvt2uu-
  ...
  **************************************************/
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T - 1; ix0++) {
                ...
                // Normalization with 0.5/GLB_VOL3
                _complex_mulr(gvt1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              gvt1_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
    ...
    for (s2 = 0; s2 < 4 * NF; s2++) {
        for (s1 = 0; s1 < 4 * NF; s1++) {
            for (ix0 = 0; ix0 < T - 1; ix0++) {
                ...
                // Normalization with 0.5/GLB_VOL3
                _complex_mulr(gvt2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1], -0.5 / GLB_VOL3,
                              gvt2_ij[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T][s2][s1]);
            }
        }
    }
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T - 1; ix0++) {
        _complex_sub_assign(gvtuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt1uu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(gvtuu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt2uu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
...
gvtdd- = - gvt1dd- + gvt2dd-
  ...
  **************************************************/
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T - 1; ix0++) {
        _complex_sub_assign(gvtdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt1dd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(gvtdd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt2dd_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
...
gvtud- = - gvt1ud- + gvt2ud-
  ...
  **************************************************/
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T - 1; ix0++) {
        _complex_sub_assign(gvtud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt1ud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(gvtud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt2ud_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
...
gvtdu- = - gvt1du- + gvt2du-
  ...
  **************************************************/
    ...
    /*******   Sum over k ************/
    for (ix0 = 0; ix0 < T - 1; ix0++) {
        _complex_sub_assign(gvtdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt1du_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
        _complex_add_assign(gvtdu_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T],
                            gvt2du_m[(zerocoord[0] + ix0 - 1 + GLB_T) % GLB_T]);
    }
