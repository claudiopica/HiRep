...
#define corr_ind(px, py, pz, n_mom, tc, nm) \
    ((px) * (n_mom) * (n_mom) * (lt) * (nm) + (py) * (n_mom) * (lt) * (nm) + (pz) * (lt) * (nm) + (tc))

/* spinor_fields* are nmx4xNF arrays of spinor_field ordered([nm][color][spinor])*/
static void measure_formfactor_core(spinor_field *psi0, spinor_field *psi1, spinor_field *eta, int nm, int tau, int tf,
                                    int n_mom, int offset, int lt, int *pt) {
                        ...
                        for (x = 0; x < X; x++) {
                            for (y = 0; y < Y; y++) {
                                for (z = 0; z < Z; z++) {
                                    ...
                                    corr[_g5_ff][corr_ind(px, py, pz, n_mom, tc, nm)] +=
                                        (cpdotx * creal(tr) + spdotx * cimag(tr));
                                    ...
                                    corr[_pipig_ff_re][corr_ind(px, py, pz, n_mom, tc, nm)] +=
                                        +0.5 * (cpdotx * creal(tr) + spdotx * cimag(tr));
                                    corr[_pipig_ff_im][corr_ind(px, py, pz, n_mom, tc, nm)] +=
                                        +0.5 * (spdotx * creal(tr) - cpdotx * cimag(tr));
                                    ...
                                    for (a = 0; a < NF; a++) {
                                        for (beta = 0; beta < 4; beta++) {
                                            _propagator_assign(spf, *_FIELD_AT(&psi1[4 * a * nm + beta * nm + i], ixmu), a,
                                                               beta);
                                        }
                                    }
                                    ...
                                    for (a = 0; a < NF; a++) {
                                        for (beta = 0; beta < 4; beta++) {
                                            _propagator_assign(spf, *_FIELD_AT(&psi0[4 * a * nm + beta * nm + i], ixmu), a,
                                                               beta);
                                        }
                                    }
                                    ...
                                    corr[_pipig_conserved_ff][corr_ind(px, py, pz, n_mom, tc, nm)] +=
                                        0.5 * (cpdotx * creal(tr) + spdotx * cimag(tr));
                                } //END SPATIAL LOOP
                            }
                        }
    ...
    for (px = 0; px < n_mom; ++px) {
        for (py = 0; py < n_mom; ++py) {
            for (pz = 0; pz < n_mom; ++pz) {
                for (i = 0; i < nm; i++) {
                    if (n_mom > 1) {
                        lprintf("MAIN", 0, "conf #%d mass=%2.6f %s FORM_FACTOR %s momentum(%d,%d,%d)= ", conf, mass[i], label,
                                ff_channel_names[channel], px, py, pz);
                    } else {
                        lprintf("MAIN", 0, "conf #%d mass=%2.6f %s FORM_FACTOR %s= ", conf, mass[i], label,
                                ff_channel_names[channel]);
                    }
                    for (t = 0; t < lt; ++t) {
                        lprintf("MAIN", 0, "%e ", corr[channel][corr_ind(px, py, pz, n_mom, t + i * lt, nm)]);
                    }
                    lprintf("MAIN", 0, "\n");
                    fflush(stdout);
                }
...
void print_formfactor(int conf, int nm, double *mass, int n_mom, char *label, int tf) {
    ...
    init = 0;
    char str[256];
    sprintf(str, "%s tf %d", label, tf); //Modify label
