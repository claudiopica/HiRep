    ...
    for (g[1]=0;g[1]<GLB_X;++g[1]) {
      for (g[2]=0;g[2]<GLB_Y;++g[2]) {
        ...
        for (p[3]=0;p[3]<NP_Z;++p[3]) { /* loop over processors in Z direction */
	      ...
	      error(mpiret != MPI_SUCCESS, -1, "write_spinor_field " __FILE__,
		    "Cannot send buffer");
	    } else {
...
void read_spinor_field(char filename[], spinor_field *sp) 
{
  ...
  if(PID==0) {
    ...
    error(local_err, -1, "read_spinor_field " __FILE__, "Gauge group mismatch");
    ...
    error(local_err, -1, "read_spinor_field " __FILE__,
    ...
    for (g[1]=0;g[1]<GLB_X;++g[1]) {
      for (g[2]=0;g[2]<GLB_Y;++g[2]) {
	    ...
	    null_error();
	      ...
	      error(mpiret != MPI_SUCCESS, -1, "read_spinor_field " __FILE__,
		    "Cannot send buffer");
	    } else {
	      null_error();
#endif
