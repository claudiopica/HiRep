...
static void swapendian32(void *ptr) {
    ...
    x = (uint32_t)((x >> 24) | ((x << 8) & 0x00FF0000) | ((x >> 8) & 0x0000FF00) | (x << 24));
...
static void swapendian64(void *ptr) {
    ...
    x = (uint64_t)((x >> 56) | ((x << 40) & 0x00FF000000000000) | ((x << 24) & 0x0000FF0000000000) |
...
static int which_endian() {
    ...
    char c = *((char *)(&one));
    if (c == 0) {
        lprintf("IO", 100, "Big Endian on machine.\n");
    } else {
        lprintf("IO", 100, "Little Endian on machine.\n");
    }
...
int fwrite_BE_int(int *ptr, size_t n, FILE *fp) {
    ...
    if (which_endian() == _BIG_ENDIAN_HRP) {
        ...
        if (sizeof(int) * CHAR_BIT == 16) {
            for (i = 0; i < n; i++) {
                swapendian16(ptr_l + i);
            }
        } else if (sizeof(int) * CHAR_BIT == 32) {
            for (i = 0; i < n; i++) {
                swapendian32(ptr_l + i);
            }
        } else if (sizeof(int) * CHAR_BIT == 64) {
            for (i = 0; i < n; i++) {
                swapendian64(ptr_l + i);
            }
        }
