...
void read_input(input_record_t irec[], char *filename) {
    ...
    error((inputfile = fopen(filename, "r")) == NULL, 1, "read_input " __FILE__, "Failed to open input file\n");

    /* count the input parameters */
    for (crec = irec, npar = 0; crec->name != NULL; ++crec) {
        ++npar;
    }
...
ENDLOOP:
    ...
    while (npar > 0) {
        ...
        if (found[npar] == 0 && irec[npar].descr != NULL) {
            lprintf("READINPUT", 0, "Warning: input parameter [%s] not found in [%s]!\n", irec[npar].name, filename);
        }
    }
    free(found);
