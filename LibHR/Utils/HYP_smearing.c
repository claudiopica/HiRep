...
void HYP_smearing(suNg_field *out, suNg_field *in, double weight[3]) {
    ...
    for (int i = 0; i < 24; i++) {
        Vbar[i] = alloc_gtransf(&glattice);
    }
    for (int i = 0; i < 12; i++) {
        Vtilde[i] = alloc_gtransf(&glattice);
    }
        ...
        _SITE_FOR(&glattice, ixp, ix) {
            for (int i = 0; i < 12; i++) {
                ...
                for (int rho = 0; rho < 4; rho++) {
                    ...
                    iy = iup(ix, rho);
                    _suNg_times_suNg(tmp[1], *_FIELD_AT(Vbar[three_to_index[rho][nu][mu]], ix),
                                     *_FIELD_AT(Vbar[three_to_index[mu][rho][nu]], iy));
                    iy = iup(ix, mu);
        ...
        _SITE_FOR(&glattice, ixp, ix) {
            for (int mu = 0; mu < 4; mu++) {
                ...
                for (int nu = 0; nu < 4; nu++) {
                    ...
                    iy = iup(ix, nu);
                    _suNg_times_suNg(tmp[1], *_FIELD_AT(Vtilde[two_to_index[nu][mu]], ix),
                                     *_FIELD_AT(Vtilde[two_to_index[mu][nu]], iy));
                    iy = iup(ix, mu);
