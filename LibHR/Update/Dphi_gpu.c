...
__global__ void Dphi_gpu_kernel(suNf_spinor* __restrict__ out,
                            ...
                            const int vol4h,
                            const int ixp, const int MASK, const int block_start_ixp, const int block_start_iyp)
{
  suNf_spinor r;
...
__global__ void Dphi_gpu_kernel_buf(suNf_spinor* __restrict__ out,
                            ...
                            const int ixp, const int MASK, const int start, const int start_piece, const int buffer_piece)
{
  suNf_spinor r;
  suNf_spinor res;
  _vector_zero_f(r.c[0]);
  ...
  if (local_ix < vol4h) {
    ix = local_ix + start_piece;

    read_gpu_suNf_vector(vol4h, res.c[0], out, local_ix, 0);
    read_gpu_suNf_vector(vol4h, res.c[1], out, local_ix, 1);
    read_gpu_suNf_vector(vol4h, res.c[2], out, local_ix, 2);
    read_gpu_suNf_vector(vol4h, res.c[3], out, local_ix, 3);
    ...
    /******************************* direction +0 *********************************/
      ...
      if (local_iy < buf_stride && local_iy >= 0) {
        read_gpu_suNf_vector(buf_stride, sn.c[0], in, local_iy, 0);
        read_gpu_suNf_vector(buf_stride, sn.c[1], in, local_iy, 2);
        read_gpu_suNf(vol4h, u, gauge_ixp, local_ix, 0);
        ...
        read_gpu_suNf_vector(buf_stride, sn.c[0], in, local_iy, 1);
        read_gpu_suNf_vector(buf_stride, sn.c[1], in, local_iy, 3);
        _vector_add_assign_f(sn.c[0], sn.c[1]);
    ...
    /******************************* direction -0 *********************************/
      ...
      if (local_iy < buf_stride && local_iy >= 0) {
        read_gpu_suNf_vector(buf_stride, sn.c[0], in, local_iy, 0);
        read_gpu_suNf_vector(buf_stride, sn.c[1], in, local_iy, 2);
        read_gpu_suNf(buf_stride, u, gauge_iyp, local_iy, 0);
    ...
    /******************************* direction -1 *********************************/
      ...
      if (local_iy < buf_stride  && local_iy >= 0) {
        read_gpu_suNf_vector(buf_stride, sn.c[0], in, local_iy, 0);
        read_gpu_suNf_vector(buf_stride, sn.c[1], in, local_iy, 3);
        read_gpu_suNf(buf_stride, u, gauge_iyp, local_iy, 1);
    ...
    /******************************* direction -2 *********************************/
      ...
      if (local_iy < buf_stride  && local_iy >= 0) {
        ...
        read_gpu_suNf(buf_stride, u, gauge_iyp, local_iy, 2);
        _suNf_theta_Y_inverse_multiply(sn.c[1], u, sn.c[0]);
    ...
    /******************************* direction -3 *********************************/
      ...
      if (local_iy < buf_stride  && local_iy >= 0) {
        ...
        read_gpu_suNf(buf_stride, u, gauge_iyp, local_iy, 3);
        _suNf_theta_Z_inverse_multiply(sn.c[1], u, sn.c[0]);
    ...
    printf("ix: %d, GPU spinor comp res END: %0.2e + i%0.2e\n", ix, 
              creal(res.c[0].c[0]), cimag(res.c[0].c[0]));
    // need to add instead of overwrite for buffers
    write_gpu_suNf_vector(vol4h, res.c[0], out, local_ix, 0);
    write_gpu_suNf_vector(vol4h, res.c[1], out, local_ix, 1);
    write_gpu_suNf_vector(vol4h, res.c[2], out, local_ix, 2);
    write_gpu_suNf_vector(vol4h, res.c[3], out, local_ix, 3);
  }
}
...
void Dphi_gpu_(spinor_field *out, spinor_field *in)
{
  ...
  _PIECE_FOR(out->type, ixp)
  {
      ...
      int start_piece_iyp = in->type->master_start[iyp];
      Dphi_gpu_kernel<<<grid, BLOCK_SIZE>>>(_GPU_FIELD_BLK(out, ixp),
                                            ...
                                            N, ixp, 0, start_piece, start_piece_iyp);
      CudaCheckError();
  }
  ...
  {
      int ixp = (i%2==0);
      int block_stride = in->type->master_end[ixp] - in->type->master_start[ixp] + 1;
      int buffer_stride = in->type->rbuf_len[i];
      int buffer_start = in->type->rbuf_start[i];
      ...
      Dphi_gpu_kernel_buf<<<grid, BLOCK_SIZE>>>(_GPU_FIELD_BLK(out, ixp),
                                                ...
                                                ixp, FULL_MASK, buffer_start, start_piece, i);
      CudaCheckError();
  }
  #endif
}
