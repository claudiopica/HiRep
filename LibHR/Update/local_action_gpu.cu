...
__global__ void _loc_action_hmc(suNg_algebra_vector *momenta, suNg_vector *momenta_s, double *loc_action, int N, int block_start)
{
    for (int id = blockDim.x * blockIdx.x + threadIdx.x; id < N; id += gridDim.x * blockDim.x)
    {
        const int ix = id + block_start;
...
void local_hmc_action_gpu(local_action_type type,
                          ...
                          suNg_scalar_field *momenta_s)
{
    ...
    _PIECE_FOR(&glattice, ixp)
    {
        ...
        if (u_scalar != NULL)
        {
            scalar_momenta = momenta_s->gpu_ptr;
        }
