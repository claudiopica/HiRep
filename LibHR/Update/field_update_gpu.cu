...
__global__ void field_update_kernel(suNg *gfield, suNg_algebra_vector *force, int N, int block_start, double dt) {
    suNg_algebra_vector f;
    suNg u;
    for (int id = blockIdx.x * blockDim.x + threadIdx.x; id < N; ++id) {
        const int ix = id + block_start;
        for (int comp = 0; comp < 4; comp++) {
            read_gpu<double>(0, &u, gfield, ix, comp, 4);
            read_gpu<double>(0, &f, force, ix, comp, 4);
            ExpX(dt, &f, &u);
            write_gpu<double>(0, &u, gfield, ix, comp, 4);
            write_gpu<double>(0, &f, force, ix, comp, 4);
        }
    }
}

void exec_field_update(suNg_field *gfield, suNg_av_field *force, double dt) {
    _PIECE_FOR(&glattice, ixp) {
        ...
        field_update_kernel<<<grid, BLOCK_SIZE>>>(gfield->gpu_ptr, force->gpu_ptr, N, block_start, dt);
    }
}
