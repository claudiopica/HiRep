...
void gfield_copy_to_gpu(suNg_field *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_gauge*4*sizeof(suNg),cudaMemcpyHostToDevice);
  free_gfield(tmp);
}

void gfield_copy_from_gpu(suNg_field *field){
  suNg_field *tmp=alloc_gfield(field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_gauge*4*sizeof(suNg),cudaMemcpyDeviceToHost);
...
void gfield_copy_to_gpu_f(suNf_field *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_gauge*4*sizeof(suNf),cudaMemcpyHostToDevice);
  free_gfield_f(tmp);
}

void gfield_copy_from_gpu_f(suNf_field *field){
  suNf_field *tmp=alloc_gfield_f(field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_gauge*4*sizeof(suNf),cudaMemcpyDeviceToHost);
...
void gfield_copy_to_gpu_flt(suNg_field_flt *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_gauge*4*sizeof(suNg_flt),cudaMemcpyHostToDevice);
	free_gfield_flt(tmp);
}

void gfield_copy_from_gpu_flt(suNg_field_flt *field){
  suNg_field_flt *tmp=alloc_gfield_flt(field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_gauge*4*sizeof(suNg_flt),cudaMemcpyDeviceToHost);
...
void gfield_copy_to_gpu_f_flt(suNf_field_flt *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_gauge*4*sizeof(suNf_flt),cudaMemcpyHostToDevice);
	free_gfield_f_flt(tmp);
}

void gfield_copy_from_gpu_f_flt(suNf_field_flt *field){
  suNf_field_flt *tmp=alloc_gfield_f_flt(field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_gauge*4*sizeof(suNf_flt),cudaMemcpyDeviceToHost);
...
void spinor_field_copy_to_gpu_f(spinor_field *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_spinor*sizeof(suNf_spinor),cudaMemcpyHostToDevice);
  free_spinor_field_f(tmp);
}

void spinor_field_copy_from_gpu_f(spinor_field *field){
  spinor_field *tmp = alloc_spinor_field_f(1, field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_spinor*sizeof(suNf_spinor),cudaMemcpyDeviceToHost);
...
void spinor_field_copy_to_gpu_f_flt(spinor_field_flt *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_spinor*sizeof(suNf_spinor_flt),cudaMemcpyHostToDevice);
  free_spinor_field_f_flt(tmp);
}

void spinor_field_copy_from_gpu_f_flt(spinor_field_flt *field){
  spinor_field_flt *tmp = alloc_spinor_field_f_flt(1, field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_spinor*sizeof(suNf_spinor_flt),cudaMemcpyDeviceToHost);
...
void sfield_copy_to_gpu(scalar_field *field){
  cudaMemcpy(field->gpu_ptr,field->ptr,field->type->gsize_spinor*sizeof(double),cudaMemcpyHostToDevice);
}
void sfield_copy_from_gpu(scalar_field *field){
  cudaMemcpy(field->ptr,field->gpu_ptr,field->type->gsize_spinor*sizeof(double),cudaMemcpyDeviceToHost);
}
...
void suNg_av_field_copy_to_gpu(suNg_av_field *field){
  ...
  cudaMemcpy(field->gpu_ptr,tmp->ptr,field->type->gsize_gauge*4*sizeof(suNg_algebra_vector),cudaMemcpyHostToDevice);
  free_avfield(tmp);
}

void suNg_av_field_copy_from_gpu(suNg_av_field *field){
  suNg_av_field* tmp = alloc_avfield(field->type);
  cudaMemcpy(tmp->ptr,field->gpu_ptr,field->type->gsize_gauge*4*sizeof(suNg_algebra_vector),cudaMemcpyDeviceToHost);
