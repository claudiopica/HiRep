...
#define _PRINT_RESULT(_name, _ninputs, _noutputs, _elapsed, _n_reps, _flopsite, _in)                            \
    do {                                                                                                        \
        ...
        lprintf("RESULT", 0,                                                                                    \
                ...
                "GFLOPS: %1.6g, "                                                                               \
                "BAND: %1.6g GB/s\n",                                                                           \
                _name, _n_reps, _elapsed, _elapsed / _n_reps * 1000., (vol_reps * _flopsite) / _elapsed / 1.e6, \
...
int main(int argc, char *argv[]) {
    ...
    _SPEED_TEST_LIN_ALG("Spinor field product", 1, 1, NF * 4, in, spinor_field_prod_f(&in[0], &in[1]););

    _SPEED_TEST_LIN_ALG("Square norm", 1, 0, NF * 4, in, spinor_field_sqnorm_f(&in[0]););

    _SPEED_TEST_LIN_ALG("g5 application", 1, 1, NF * 4, in, spinor_field_g5_f(&in[0], &in[1]););

    _SPEED_TEST_LIN_ALG("g5 mulc add assign", 1, 1, NF * 4, in, spinor_field_g5_mulc_add_assign_f(&in[0], c, &in[1]););
    ...
    _SPEED_TEST_LIN_ALG_FLT("Square norm single precision", 1, 0, NF * 4, in_flt, spinor_field_sqnorm_f_flt(&in_flt[0]););
