...
#define _SPEED_TEST_RNG(_random, _random_init, _type) \
    do { \
        ...
        _random_init(1, 42); \
        timer_lap(&clock); \
        for (int i = 0; i < n_warmup; ++i) { _random(r, n_batch); synchronize; } \
        double elapsed = timer_lap(&clock) * 1.e-3; \
        \
        int n_reps = (int)(n_warmup * 1.01 * (time_target / elapsed)); \
        global_sum_int(&n_reps, 1); \
        \
        lprintf("WARMUP", 0, "Needed %lf msec to generate %d random numbers. " \
                            "Time single: %lf usec\n",  \
                            n_warmup * n_batch,  \
                            elapsed, \
                            elapsed / n_warmup / n_batch * 1000.); \
        \
        int i = 0; \
        do { \
            lprintf("LA TEST", 0, "Trying reps: %d\n", n_reps); \
            timer_lap(&clock); \
            for (int i = 0; i < n_reps; ++i) { _random(r, n_batch); synchronize; } \
        ...
        lprintf("RESULT", 0, "RNG speed: %d samples generated in %lf, " \
...
#define _SPEED_TEST_RANDOM_FIELD(_name, _in, _test) \
    do { \
        ...
        lprintf("RESULT", 0, "%s reps: %d, " \
                            ...
                            _name, \
                            n_reps, \
                            elapsed, elapsed / n_reps * 1000., \
                            (vol_reps * bytesite) / elapsed / 1.e6); \
    } while (0);
...
int main(int argc, char *argv[]) {
    ...
    _SPEED_TEST_RANDOM_FIELD("Gaussian spinor field", in, 
        gaussian_spinor_field(in);
    );

    spinor_field_flt *in_flt = alloc_spinor_field_f_flt(1, &glattice);
    _SPEED_TEST_RANDOM_FIELD("Gaussian spinor field single precision", in_flt, 
        gaussian_spinor_field_flt(in_flt);
    );

    suNg_field *u = alloc_gfield(&glattice);
    _SPEED_TEST_RANDOM_FIELD("Random gauge field", in, 
        random_u(u);
    );

    suNg_av_field *avf = alloc_avfield(&glattice);
    _SPEED_TEST_RANDOM_FIELD("Gaussian momenta", in, 
        gaussian_momenta(avf);
    );

    suNg_scalar_field *sf = alloc_suNg_scalar_field(&glattice);
    _SPEED_TEST_RANDOM_FIELD("Gaussian scalar momenta", in, 
        gaussian_scalar_momenta(sf);
    );
