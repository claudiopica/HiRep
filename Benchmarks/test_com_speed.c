...
static void transform_s(suNg_field *suNg_field, spinor_field *in) {
    ...
    _MASTER_FOR(&glattice, ix) {
        ...
        _group_represent2(&gfx, _FIELD_AT(suNg_field, ix));
...
int main(int argc, char *argv[]) {
    ...
    s1 = alloc_spinor_field(1, &glattice);
    ...
    lprintf(
        "MAIN", 0,
        "Reference: the number of flop per site in the actual setup for the application of the plain Dirac operator to a full spinor is %ld \n",
        flopsite);
    lprintf("MAIN", 0,
            "The number of flop in your bulk in the actual setup for the application of the plain Dirac operator is %ld \n",
    ...
    for (k = 0; k < repts; k++) {
        ...
        start_sendrecv_spinor_field(s1);

        complete_sendrecv_spinor_field(s1);
    ...
    for (k = 0; k < intervals + 1; k++) {
        ...
        lprintf("MAIN", 0,
                "start_sf_send + bulk of %ld flop ops + complete_sf_recv : time in [%lf millisec] +- [%lf millisec]\n",
                steps[k] * singlestep, elapsed_mean, elapsed_var);
    }
    ...
    for (k = 0; k < intervals + 1; k++) {
        ...
        lprintf("MAIN", 0,
                "start_gf_send + bulk of %ld flop ops + complete_gf_recv : time in [%lf millisec] +- [%lf millisec]\n",
                steps[k] * singlestep, elapsed_mean, elapsed_var);
    }
