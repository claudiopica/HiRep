...
#define X(NAME) lprintf("DEBUG",0,"Creating propagator Q_%s \n", #NAME);\
    ...
    SOURCE_LIST_Q
#undef X
#define X(NAME) lprintf("DEBUG",0,"Creating propagator W_%s \n", #NAME); spinor_field* W_##NAME = alloc_spinor_field_f(4*NF,&glattice); spinor_field* W_##NAME##_A = alloc_spinor_field_f(4*NF,&glattice);
    SOURCE_LIST_W
#undef X
...
#define X(NAME) calc_propagator(W_##NAME, source_##NAME,4);\
    flip_T_bc(tau);\
    calc_propagator(W_##NAME##_A, source_##NAME,4);\
    flip_T_bc(tau);\
    for(l=0;l<4;++l){\
        spinor_field_add_assign_f(&W_##NAME[l],&W_##NAME##_A[l]);\
        spinor_field_mul_f(&W_##NAME[l],0.5,&W_##NAME[l]);\
    }
...
#undef X
        ...
        flip_T_bc(tau);
		calc_propagator(W_0_0_A, source_0_0, 1);
        flip_T_bc(tau);
        for(l=0;l<4;++l){
            spinor_field_add_assign_f(&W_0_0[l],&W_0_0_A[l]);
            spinor_field_mul_f(&W_0_0[l],0.5,&W_0_0[l]);
        }
