...
## TODO: Prolog, Epilon, Macro docs, pl script docs
...
sub write_gpu_geometry_functions {
    ...
    for my $prec (@a) { # generate for different precisions
        for my $repr (@a) { # generate for different representations
            ...
            for ($i=0; $i<$N-2; $i+=2) {
                print "\t\tcreal((v).c\[__iz\])=((${type}*)(in))\[__iz\]; __iz+=(stride); \\\n";
                print "\t\tcimag((v).c\[__iz\])=((${type}*)(in))\[__iz\]; __iz+=(stride); \\\n";
            }
            print "\t\tcreal((v).c\[__iz\])=((${type}*)(in))\[__iz\]; __iz+=(stride); \\\n";
            print "\t\tcimag((v).c\[__iz\])=((${type}*)(in))\[__iz\]; \\\n";
            print "\t} while (0) \n\n";
            ...
            print "\t\tint __iz = (ix) + ((comp)*$N)*(stride); \\\n";
            for ($i=0; $i<$N-2; $i+=2) {
                print "\t\t((${type}*)(out))\[__iz\]=creal((v).c\[__iz\]); __iz+=(stride);\\\n";
                print "\t\t((${type}*)(out))\[__iz\]=cimag((v).c\[__iz\]); __iz+=(stride);\\\n";
            }
            print "\t\t((${type}*)(out))\[__iz\]=creal((v).c\[__iz\]); __iz+=(stride);\\\n";
            print "\t\t((${type}*)(out))\[__iz\]=cimag((v).c\[__iz\]);\\\n";
            print "\t} while (0) \n\n";
        }
    }
}
