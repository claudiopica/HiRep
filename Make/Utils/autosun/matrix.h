...
public:
    pmatrix()
        : sparsematrix<polynomial>() {
    }
    pmatrix(int N)
        : sparsematrix<polynomial>(N) {
    }
    pmatrix(smatrix &mat)
        : sparsematrix<polynomial>(mat.size) {
...
public:
    rmatrix(int N, const string &name)
        : pmatrix(N) {
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                ostringstream elem;
...
public:
    cmatrix(int N, const string &name)
        : pmatrix(N) {
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                ostringstream elem;
...
public:
    pvector()
        : sparsevector<polynomial>() {
    }
    pvector(int N)
        : sparsevector<polynomial>(N) {
    }
    pvector(svector &vec)
        : sparsevector<polynomial>(vec.size) {
        static rmonomial index;
...
public:
    rvector(int N, const string &name)
        : pvector(N) {
        for (int i = 0; i < N; i++) {
            ostringstream elem;
...
public:
    cvector(int N, const string &name)
        : pvector(N) {
        for (int i = 0; i < N; i++) {
            ostringstream elem;
            elem << name << vindex(i, size);
