...
public:
    ...
    sparselinear(const sparselinear<INDEX, VALUE> &mat)
        : orderedlist<INDEX, VALUE>(mat) {
        size = mat.size;
    }
...
struct mindex {
    ...
    friend bool operator<(const mindex &a, const mindex &b) {
        return a.key() < b.key();
    }
    friend bool operator>(const mindex &a, const mindex &b) {
        return a.key() > b.key();
...
public:
    sparsematrix()
        : sparselinear<mindex, VALUE>() {
    }
    sparsematrix(int n)
        : sparselinear<mindex, VALUE>(n) {
    }
    sparsematrix(const sparsematrix<VALUE> &mat)
        : sparselinear<mindex, VALUE>(mat) {
    }
    ...
    friend void trace(VALUE &ret, const sparsematrix<VALUE> &l) {
        ret.clear();
        for (int i = 0; i < l.length; i++) {
            if (l[i].index.row == l[i].index.col) { ret.add(l[i].value); }
        }
    }
};
...
struct vindex {
    ...
    friend bool operator<(const vindex &a, const vindex &b) {
        return a.row < b.row;
    }
    friend bool operator>(const vindex &a, const vindex &b) {
        return a.row > b.row;
...
public:
    sparsevector()
        : sparselinear<vindex, VALUE>() {
    }
    sparsevector(int n)
        : sparselinear<vindex, VALUE>(n) {
    }
    sparsevector(const sparsevector<VALUE> &vec)
        : sparselinear<vindex, VALUE>(vec) {
    }
