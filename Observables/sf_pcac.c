...
{
  int i,j,ix0,ix1,ix2,ix3,source;
  ...
  S_top[0]=malloc((4*NF)*sizeof(suNf_spinor));
  S_bottom[0]=malloc((4*NF)*sizeof(suNf_spinor));
  _spinor_zero_f(*S_top[0]);
  _spinor_zero_f(*S_bottom[0]);
  for(j=1;j<4*NF;j++){
    S_top[j]=S_top[0]+j;
    _spinor_zero_f(*S_top[j]);
    S_bottom[j]=S_bottom[0]+j;
