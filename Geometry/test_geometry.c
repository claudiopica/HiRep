...
void test_geometry_descriptor(geometry_descriptor *gd, int(*test)(int*)) {
	...
	for(i=0; i<gd->ncopies; i++) {
		for(j=0; j<gd->copy_len[i]; j++) {
			x = gd->copy_from[i]+j;
			y = gd->copy_to[i]+j;
			if(x<0 || x>=gd->gsize || y<0 || y>=gd->gsize) {
	...
	for(i=0; i<inner_pieces; i++) {
		for(x=gd->master_start[i]; x<= gd->master_end[i] ; x++) {
			if(x<0 || x>=gd->gsize) {
				lprintf("TEST_GEOMETRY.C",0,"gd->master_start[%d]+%d = %d (gd->len[%d]=%d).\n",i,j,x,i,gd->master_end[i]- gd->master_start[i] + 1);
				test_q = false;
			} else {
				if(descr[x].t_flag == 1) {
...
#define LOCAL 100
		...
		for(j=0; j<gd->buf_len[i]; j++) {
			x = gd->sbuf_start[i]+j;
			for(k=0;k<4;k++) {
				if(descr[x].b_type[k] == LBUFFER || descr[x].b_type[k] == RBUFFER) {
					lprintf("TEST_GEOMETRY.C",loglevel,"gd->sbuf_start[%d]+%d=%x but it is in a buffer.\n",i,j,x);
					test_q = false;
		...
		for(j=0; j<gd->buf_len[i]; j++) {
			...
			for(k=0;k<4;k++) {
				if(rbuf_mask[k] == RBUFFER && descr[x].b_type[k] != RBUFFER) {
					lprintf("TEST_GEOMETRY.C",loglevel,"rbuf_mask[%d]=RBUFFER but coord[%d=gd->rbuf_start[%d]+%d]=(%d,%d,%d,%d).\n",
					        k,x,i,j,cx[0],cx[1],cx[2],cx[3]);
					test_q = false;
				} else if(rbuf_mask[k] == LBUFFER && descr[x].b_type[k] != LBUFFER) {
					lprintf("TEST_GEOMETRY.C",loglevel,"rbuf_mask[%d]=LBUFFER but coord[%d=gd->rbuf_start[%d]+%d]=(%d,%d,%d,%d).\n",
					        k,x,i,j,cx[0],cx[1],cx[2],cx[3]);
					test_q = false;
				} else if(rbuf_mask[k] == LOCAL && (descr[x].b_type[k] == LBUFFER || descr[x].b_type[k] == RBUFFER)) {
					lprintf("TEST_GEOMETRY.C",loglevel,"rbuf_mask[%d]=LOCAL but coord[%d=gd->rbuf_start[%d]+%d]=(%d,%d,%d,%d).\n",
					        k,x,i,j,cx[0],cx[1],cx[2],cx[3]);
					test_q = false;
				}
			}
		...
		for(c3=sid_crange[0][3]; c3<=sid_crange[1][3]; c3++) {
			int isthere_q = false;
			for(j=0; j<gd->buf_len[i] && isthere_q == false; j++) {
				x = gd->sbuf_start[i]+j;
				cx = coord[x];
				if(c0 == cx[0] && c1 == cx[1] && c2 == cx[2] && c3 == cx[3])
					isthere_q = true;
		...
		if(gd->rbuf_from_proc[i] == myid && !periodic_q[i]) {
		...
		if(gd->rbuf_from_proc[i] < 0 || gd->rbuf_from_proc[i] >= np_x*np_y*np_z*np_t) {
			lprintf("TEST_GEOMETRY.C",loglevel,"rbuf_from_proc[%d]=%d (rbuf_mask={%d,%d,%d,%d}) out of range.\n", i,gd->rbuf_from_proc[i],rbuf_mask[0],rbuf_mask[1],rbuf_mask[2],rbuf_mask[3]);
			test_q = false;
		}
		id = myid;
		for(k=0; k<4; k++) {
		...
		if(id != gd->rbuf_from_proc[i]) {
			lprintf("TEST_GEOMETRY.C",loglevel,"rbuf_from_proc[%d]=%d (rbuf_mask={%d,%d,%d,%d}) , expected %d.\n", i,gd->rbuf_from_proc[i],rbuf_mask[0],rbuf_mask[1],rbuf_mask[2],rbuf_mask[3],id);
			test_q = false;
		}

		/* TEST: sid*[i] is sent to the right processor */
		if(gd->sbuf_to_proc[i] == myid && !periodic_q[i]) {
		...
		if(gd->sbuf_to_proc[i] < 0 || gd->sbuf_to_proc[i] >= np_x*np_y*np_z*np_t) {
			lprintf("TEST_GEOMETRY.C",loglevel,"sbuf_to_proc[%d]=%d (sbuf_mask={%d,%d,%d,%d}) out of range.\n", i,gd->sbuf_to_proc[i],sbuf_mask[0],sbuf_mask[1],sbuf_mask[2],sbuf_mask[3]);
			test_q = false;
		}
		id = myid;
		for(k=0; k<4; k++) {
		...
		if(id != gd->sbuf_to_proc[i]) {
			lprintf("TEST_GEOMETRY.C",loglevel,"sbuf_to_proc[%d]=%d (sbuf_mask={%d,%d,%d,%d}) , expected %d.\n", i,gd->sbuf_to_proc[i],sbuf_mask[0],sbuf_mask[1],sbuf_mask[2],sbuf_mask[3],id);
			test_q = false;
		}
