    ...
    for (x1=0;x1<X;x1++){
			for (x2=0;x2<Y;x2++){
				for (x3=0;x3<Z;x3++){
					...
					ckdir[1] = ipt(x0,safe_mod(x1+1,X),x2,x3);
					ckdir[2] = ipt(x0,x1,safe_mod(x2+1,Y),x3);
					ckdir[3] = ipt(x0,x1,x2,safe_mod(x3+1,Z));
					for (mu=0;mu<4;++mu){
		...
		for (x1=0;x1<X;x1++){
			for (x2=0;x2<Y;x2++){
				for (x3=0;x3<Z;x3++){
					...
					n =iup(ix,1);
					find_coord(n, &x0r, &x1r, &x2r, &x3r);
					if ((x0!=x0r)||(safe_mod(x1+1,X)!=x1r)||(x2!=x2r)||(x3!=x3r))
					...
					n =iup(ix,2);
					find_coord(n, &x0r, &x1r, &x2r, &x3r);
					if ((x0!=x0r)||(x1!=x1r)||(safe_mod(x2+1,Y)!=x2r)||(x3!=x3r))
					...
					n =iup(ix,3);
					find_coord(n, &x0r, &x1r, &x2r, &x3r);
					if ((x0!=x0r)||(x1!=x1r)||(x2!=x2r)||(safe_mod(x3+1,Z)!=x3r))
