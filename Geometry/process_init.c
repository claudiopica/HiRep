...
int setup_replicas() {
  ...
  /* set up replicas */
  N_REP=REP_T*REP_X*REP_Y*REP_Z;
  if (N_REP>1) {
    char sbuf[64];
    if ((MPI_WORLD_SIZE%N_REP)!=0) {
      error(1,1,"setup_process " __FILE__,"MPI_WORLD_SIZE is not a multiple of the number of replicas!");
    }
    ...
    if (mpiret != MPI_SUCCESS) {
      ...
      MPI_Error_string(mpiret,mesg,&mesglen);
      lprintf("MPI",0,"ERROR: %s\n",mesg);
      error(1,1,"setup_process " __FILE__,"Inizialization of replicas failed");
    }
