/***************************************************************************\
* Copyright (c) 2008, 2011 and 2012 Claudio Pica and Ari Hietanen           *   
* All rights reserved.                                                      * 
\***************************************************************************/
...
if (type->nbuffers>0) {\
f->comm_req=amalloc(n*2*type->nbuffers*sizeof(MPI_Request),ALIGN);\
error((f->comm_req)==NULL,1,"alloc_" #_name " [" __FILE__ "]",\
"Could not allocate memory space for field (MPI)");\
for (int ix=0; ix<n*2*type->nbuffers; ++ix)\
f->comm_req[ix]=MPI_REQUEST_NULL;\
for (int i=1; i<n; ++i) f[i].comm_req=f[i-1].comm_req+2*type->nbuffers;\
} else {\
...
cudaError_t err;\
err = alloc_pool_gpu((void **) &(f->gpu_ptr), n*_size*type->gsize*sizeof(*(f->gpu_ptr))); \
...
for (int i=1; i<n; ++i) f[i].ptr=f[i-1].ptr+type->gsize*_size;\
