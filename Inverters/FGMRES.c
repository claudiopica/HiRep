...
static int GMRES_core_flt(short int *valid, inverter_par *par,  double inorm , spinor_operator M, spinor_field_flt *in, spinor_field_flt *out, spinor_field_flt *trial){
  int kry_dim = par->kry_dim;
  int i,j,k;
  int cgiter = 0;
  double beta;
...
int FGMRES_flt(inverter_par *par, spinor_operator M, spinor_field_flt *in, spinor_field_flt *out, spinor_field_flt *trial){
  ...
  iter = GMRES_core_flt(&valid,par,inorm,M,in,out,NULL);
  while (!valid && (par->max_iter==0 || iter < par->max_iter)){
    iter += GMRES_core_flt(&valid,par,inorm,M,in,out,out);
    if ((++rep %10 == 0 )){
      lprintf("INVERTER",-10,"GMRES recursion = %d (precision too high?)\n",rep);
    }
  }
