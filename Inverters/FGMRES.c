...
static int FGMRES_core(short int *valid, inverter_par *par, double inorm , spinor_operator M, spinor_field *in, spinor_field *out, spinor_field *trial,spinor_operator precon){
...
#endif
  v = alloc_spinor_field_f(2*kry_dim+1,in->type);
  z = v+kry_dim;
  w = z+kry_dim;
  ...
  lprintf("INVERTER",100,"FGMRES Initial norm: %e\n",beta);
  ...
  for (j=0;j<kry_dim;++j){
    ...
    if ( norm_w/tmp < 0.0001){
     lprintf("INVERTER",100,"FGMRES in second GS\n");
     for (i=0;i<j+1;++i){
    ...
    if (norm_w*norm_w < 1e-28){
      lprintf("INVERTER",500,"FGMRES_core: Lucky termination!:\n");
      break;
    }
    ...
    lprintf("INVERTER",100,"FGMRES Error in iteration %d: %e\n",j,gbar[j+1].re*gbar[j+1].re/inorm);
    /*Check Convergence*/
  ...
  for (i=0;i<j;++i){
    spinor_field_mulc_add_assign_f(out,cs[i],&z[i]);
  }
  
  lprintf("INVERTER",50,"FGMRES Error after FGMRES_core: %e\n",gbar[j].re*gbar[j].re/inorm);
...
static int FGMRES_core_flt(short int *valid, inverter_par *par,  double inorm , spinor_operator M, spinor_field_flt *in, spinor_field_flt *out, spinor_field_flt *trial){
  int kry_dim = par->kry_dim;
  int i,j,k;
  int cgiter = 0;
  double beta;
  ...
  for (j=0;j<kry_dim;++j){
    ...
    for (i=0;i<j+1;++i){	// Gram-Schmidth orthogonalization (modified)
      ...
      _complex_minus(c_tmp1,h[Hind(i,j)]);
      c_tmp1_flt.re=(float)c_tmp1.re;c_tmp1_flt.im=(float)c_tmp1.im;
      spinor_field_mulc_add_assign_f_flt(w,c_tmp1_flt,&v[i]);
    }
  ...
  for (i=0;i<j;++i){
    ...
    c_tmp1_flt.re=(float)cs[i].re;c_tmp1_flt.im=(float)cs[i].im;
    spinor_field_mulc_add_assign_f_flt(out,c_tmp1_flt,&v[i]);
  }
...
int FGMRES(inverter_par *par, spinor_operator M, spinor_field *in, spinor_field *out, spinor_field *trial, spinor_operator precon){
  ...
  short int valid=0;
  unsigned long int iter_dbl = getMVM();
  unsigned long int iter_flt = getMVM_flt();
  iter = FGMRES_core(&valid,par,inorm,M,in,out,NULL,precon);
  while (!valid && (par->max_iter==0 || iter < par->max_iter)){
    iter += FGMRES_core(&valid,par,inorm,M,in,out,out,precon);
    if ((++rep %10 == 0 )){
      lprintf("INVERTER",-10,"FGMRES recursion = %d (precision too high?)\n",rep);
    }
  }
  lprintf("INVERTER",10,"FGMRES Dphi calls: %d, %d\n",getMVM()-iter_dbl,getMVM_flt()-iter_flt);
  return iter;
}


int FGMRES_flt(inverter_par *par, spinor_operator M, spinor_field_flt *in, spinor_field_flt *out, spinor_field_flt *trial){
  int iter, rep=0;
  double inorm = spinor_field_sqnorm_f_flt(in);
  short int valid=0;
  
  iter = FGMRES_core_flt(&valid,par,inorm,M,in,out,NULL);
  while (!valid && (par->max_iter==0 || iter < par->max_iter)){
    iter += FGMRES_core_flt(&valid,par,inorm,M,in,out,out);
    if ((++rep %10 == 0 )){
