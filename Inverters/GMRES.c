...
// This version might not work... I can't get into it right now. There should be a working version in the GPU repository
static int GMRES_core(short int *valid, inverter_par *par, spinor_operator M, spinor_field *in, spinor_field *out, spinor_field *trial){
  ...
  spinor_field * w;
  spinor_field * v;
  double  h[kry_dim*(kry_dim+1)/2];
  double ss[kry_dim];
  double cs[kry_dim];
  double gbar[kry_dim+1];

  double tmp;
  double norm_w;
  double c_tmp1,c_tmp2;
  ...
  for (j=0;j<kry_dim;++j){
    ...
    for (i=0;i<j+1;++i){	// Gram-Schmidth orthogonalization (modified)
      h[Hind(i,j)]=spinor_field_prod_re_f(w,&v[i]);
      spinor_field_mul_add_assign_f(w,-h[Hind(i,j)],&v[i]);
 // lprintf("INVERTER",100,"DEBUG h[%d,%d]=: %e\n",i,j,h[Hind(i,j)]);
    }
    ...
    if ( norm_w/tmp < 0.001){
     ...
     for (i=0;i<j+1;++i){	// Gram-Schmidth orthogonalization (modified)
       c_tmp1=spinor_field_prod_re_f(w,&v[i]);
       h[Hind(i,j)]=c_tmp1;
       spinor_field_mul_add_assign_f(w,-c_tmp1,&v[i]);
     }
      norm_w = sqrt(spinor_field_sqnorm_f(w));      
    }
    ...
    for (i=0;i<j;++i){
      c_tmp1=h[Hind(i,j)];
      h[Hind(i,j)]=cs[i]*c_tmp1;
      h[Hind(i,j)]+=ss[i]*h[Hind((i+1),j)];
      c_tmp2=h[Hind((i+1),j)];
      h[Hind((i+1),j)]=-ss[i]*c_tmp1;
      h[Hind((i+1),j)]+=cs[i]*c_tmp2;
      
    }

    //Calculate new sin and cos term 
    tmp=sqrt(h[Hind(j,j)]*h[Hind(j,j)]+norm_w*norm_w);
    ss[j]= norm_w/ tmp;
    cs[j]=h[Hind(j,j)]/tmp;
    ...
    c_tmp1=h[Hind(j,j)];
    h[Hind(j,j)]=c_tmp1*cs[j];
    h[Hind(j,j)] += ss[j]*norm_w;

    lprintf("INVERTER",500,"GMRES Error in iteration %d: %e\n",j,gbar[j+1]*gbar[j+1]/inorm);
    /*Check Convergence*/
    *valid = (fabs(gbar[j+1]*gbar[j+1]/inorm) < par->err2 );
    if (*valid){
      j++;
      break;
    }
  ...
  for (i=j-1;i>=0;--i){
    cs[i]=gbar[i];
    for (k=max(j-1,i+1);k>i;--k){
      cs[i]-=h[Hind(i,k)]*cs[k];
    }
    cs[i]*=1./h[Hind(i,i)];
  }
  ...
  for (i=0;i<j;++i){
    spinor_field_mul_add_assign_f(out,cs[i],&v[i]);
  }
