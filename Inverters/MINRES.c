...
static int MINRES_core_flt(short int *valid, MINRES_par *par, spinor_operator M, spinor_field_flt *in, spinor_field_flt *out, spinor_field_flt *trial){
  ...
  beta=sqrt(spinor_field_sqnorm_f_flt(p2));
  ...
  do {
    ...
    /* compute alpha */
    alpha = spinor_field_prod_re_f_flt(Mp,p2);
  ...
  innorm2=spinor_field_sqnorm_f_flt(Mp)/innorm2;
  *valid=1;
  if (fabs(innorm2)>par->err2) {
    *valid=0;
    lprintf("INVERTER",30,"MINRES failed: err2 = %1.8e > %1.8e\n",innorm2,par->err2);
