...
//Loop over inner pieces of given type
#define _INNER_PIECE_FOR(type, ip) \
  for (int ip = 0; ip < (type)->inner_master_pieces; ip++)
...
#define _GPU_IDX_TO_GLOBAL(in, ix, ixp) (ix) + in->type-master_start[(ixp)]
