...
double h2evamass=0.;
void H2eva_dbl(spinor_field *out, spinor_field *in){
  g5Dphi_sq(h2evamass, out, in);
}
spinor_operator H2eva={&H2eva_dbl,NULL};
  ...
  for(i=flow.start;i<flow.end;++i) {
    ...
    if((i%flow.meas_freq)==0) {
      ...
      if(strcmp(eigval_var.make,"true")==0) {
        ...
        int status;
        int ie=eva(eigval_var.nev, eigval_var.nevt, 0, eigval_var.kmax, eigval_var.maxiter, max, eigval_var.omega1, eigval_var.omega2, H2eva.dbl, eva_vecs, eva_vals, &status);
        while (ie!=0) { /* if failed restart EVA */
          lprintf("MAIN",0,"Restarting EVA!\n");
          ie=eva(eigval_var.nev, eigval_var.nevt, 2, eigval_var.kmax, eigval_var.maxiter, max, eigval_var.omega1, eigval_var.omega2, H2eva.dbl, eva_vecs, eva_vals, &status);
        }
